...
import { changeSkillAudio } from "../../features/changeVoices.js";
...
const sort = {
  ...
  PScharacter_shu: [
    "PScharacter_shu",
    '<span style="color:#ff453e;font-family:xingkai;font-size:24px">汉祚延绵</span>',
  ],
  PScharacter_wu: [
    "PScharacter_wu",
...
export const data = {
      ...
      PSshuangquan: {
        ...
        content: function () {
          ...
          for (let equip of equips) {
            expand = 0;
            num =
              player.countEnabledSlot(equip) + player.countDisabledSlot(equip);
            while (expand < num) {
        ...
        subSkill: {
          ...
          clear: {
            ...
            filter: function (event, player) {
              return (
                player.storage.PSshuangquan_phase == 1 && event.player != player
              );
            },
            priority: -50,
            forced: true,
            ...
            filter: function (event, player, name) {
              ...
              return game.findPlayer2((target) => {
                ...
                if (info.autoViewAs) {
                  if (
                    player.canUse(
                      { card: event.card, name: info.autoViewAs },
                      target,
                      false
                    )
                  )
      ...
      PSjianxiong: {
        ...
        filter: function (event, player) {
          return (
            event.player != player &&
            event.cards.length &&
            ["o", "d"].includes(get.position(event.cards[0]))
          );
        },
        content: function () {
          "step 0";
  ...
  PSdianwei: {
    ...
    skills: {
      PSqiangxi: {
        ...
        filter: function (event, player) {
          if (
            player.hp < 1 &&
            !player.hasCard(
              (card) => lib.skill.PSqiangxi.filterCard(card),
              "he"
            )
          )
            return false;
          return game.hasPlayer((current) =>
            lib.skill.PSqiangxi.filterTarget(null, player, current)
          );
        },
          ...
          result: {
            ...
            target: function (player, target) {
              const num =
                ...
                lib.skill.PSqiangxi.getUseSkillTime(player);
              if (player.hasSkill("buqu") && player.hasSkill("xinjuejing"))
                return num;
      ...
      PSsizhan: {
        ...
        init: function () {
          changeSkillAudio("buqu", "PSdianwei", "juanjia");
          changeSkillAudio("xinjuejing", "PSdianwei", "qiexie");
        },
          ...
          defend: {
            ...
            content: function () {
              ...
              for (let i = history.length - 1; i >= 0; i--) {
                if (
                  history[i].useSkill.some((evt) =>
                    evt.skill.includes("PSqiangxi")
                  )
                ) {
                  const index = history[i].useSkill.findLastIndex((evt) =>
                    evt.skill.includes("PSqiangxi")
                  );
            ...
            content: function () {
              ...
              for (let i = 0; i < player.actionHistory.length; i++) {
                player.actionHistory[i].useSkill = player.actionHistory[
                  i
                ].useSkill.filter((evt) => evt.skill != "PSqiangxi");
              }
              if (_status.currentPhase === player) {
  ...
  PSsunshangxiang: {
    ...
    intro: "由“九个鲨雕”设计",
    replace: ["PSsunshangxiang", "PSshu_sunshangxiang"],
    skills: {
      PSxiaoji: {
        ...
        content: function () {
          ...
          if (event.count > 0) {
            player
              .chooseBool(get.prompt2("xiaoji"))
              .set("frequentSkill", "xiaoji").ai = lib.filter.all;
          }
          ("step 3");
          if (result.bool) {
      ...
      PSqiaobian: {
        ...
        subSkill: {
          ...
          draw: {
            ...
            prompt:
              "###是否发动【巧变·摸牌】？###摸一张牌跳过摸牌阶段，并选择获得其他一至两名角色的各一张手牌",
            filter: function (event, player) {
              return !event.numFixed;
            },
            content: function () {
          ...
          use: {
            ...
            prompt:
              "###是否发动【巧变·出牌】？###摸一张牌跳过出牌阶段，并选择移动场上一张牌",
            content: function () {
              "step 0";
              var check;
      ...
      PSpingjian: {
        ...
        initList: function () {
          ...
          game.countPlayer2(function (current) {
            ...
            if (
              current.storage.rehuashen &&
              current.storage.rehuashen.character
            )
            ...
            for (var i = 0; i < _status.characterlist.length; i++) {
              ...
              for (var j = 0; j < skills2.length; j++) {
                ...
                if (
                  ...
                  lib.skill.PSfushi.characterList().includes(name)
                ) {
                  list.add(name);
                ...
                for (var k = 0; k < list2.length; k++) {
                  ...
                  if (
                    ...
                    if (
                      info.init ||
                      (info.ai &&
                        (info.ai.combo || info.ai.notemp || info.ai.neg))
                    )
                      continue;
                    if (info.filter) {
                      try {
    ...
    skills: {
      PSqianju: {
        mod: {
          ...
          cardname: function (card, player, name) {
            if (
              lib.card[card.name].subtype == "equip3" ||
              lib.card[card.name].subtype == "equip4" ||
              lib.card[card.name].subtype == "equip6"
            )
              return "sha";
          },
      ...
      PSqingxi: {
        ...
        filter: function (event, player) {
          ...
          return (
            event.card.name == "sha" ||
            (get.type(event.card, false) == "trick" &&
        ...
        subSkill: {
          damage: {
            ...
            filter: function (event, player) {
              return (
                event.card &&
                player.storage.PSqingxi_damage.includes(event.card)
              );
            },
            content: function () {
              trigger.num++;
  ...
  PSshen_zhuge: {
    ...
    skills: {
      PSqixing: {
        ...
        filter: function (event, player) {
          return (
            player.getExpansions("PSqixing").length < 7 &&
            (event.name != "phase" || game.phaseNumber == 0)
          );
        },
        content: function () {
          ...
          player
            .addToExpansion(get.cards(7 - num), "draw")
            .gaintag.add("PSqixing");
          ("step 1");
        ...
        subSkill: {
          draw: {
            ...
            filter: function (event, player) {
              return (
                player.getExpansions("PSqixing").length > 0 &&
      ...
      PSkuangfeng: {
        ...
        content: function () {
          ...
          player.chooseTarget(
            get.prompt("PSkuangfeng"),
            "令至多" + get.cnNumber(num) + "名角色获得“狂风”标记",
            [1, num]
          ).ai = function (target) {
            return -1;
          };
          ...
          if (result.bool) {
            ...
            targets.forEach((target) => {
              target.addAdditionalSkill(
                `PSkuangfeng_${player.playerid}`,
                "PSkuangfeng_storage"
              );
              target.markAuto("PSkuangfeng_storage", [player]);
            });
            player.addTempSkill("PSkuangfeng_damage", {
              player: "phaseUseEnd",
            });
            player.chooseCardButton(
              "选择弃置" + get.cnNumber(length) + "张“星”",
              length,
              player.getExpansions("PSqixing"),
              true
            );
          } else {
            event.finish();
          ...
          damage: {
            ...
            filter: function (event, player) {
              return (
                ...
                event.player.getStorage("PSkuangfeng_storage").includes(player)
              );
            },
            charlotte: true,
            forced: true,
            ...
            onremove: function (player) {
              game.countPlayer2((current) => {
                if (
                  current.getStorage("PSkuangfeng_storage").includes(player)
                ) {
          ...
          damage: {
            ...
            filter: function (event, player) {
              return (
                ...
                event.player.getStorage("PSdawu_storage").includes(player)
              );
            },
            forced: true,
            charlotte: true,
      ...
      PSjizhi: {
        ...
        content: function () {
          ...
          for (var i = 1; i < cards.length; i++) {
            if (get.cardNameLength(cards[i]) > get.cardNameLength(cardsx[0]))
              cardsx = [cards[i]];
            else if (
              get.cardNameLength(cards[i]) == get.cardNameLength(cardsx[0])
            )
              cardsx.push(cards[i]);
          }
        ...
        subSkill: {
            ...
            content: function () {
              ...
              player.chooseTarget(
                ...
                if (
                  ...
                  (target.isTurnedOver() || target.countCards("h") < 1)
                )
                  return 0.2;
                if (
                  get.attitude(player, target) < 1 &&
                  target.countCards("h") > 0 &&
                  target.countCards("j", { name: "lebu" }) > 0
                )
                  return (
          ...
          3: {
            ...
            filter: function (event, player) {
              return (
                player != event.player &&
                event.player.hasMark("PSjieying_mark") &&
            ...
            content: function () {
              ...
              trigger.player.removeMark(
                "PSjieying_mark",
                trigger.player.countMark("PSjieying_mark")
              );
            },
            sub: true,
          },
    ...
    skills: {
      PSdunshi: {
        ...
        hiddenCard: function (player, name) {
          return (
            !player.getStorage("PSdunshi").includes(name) &&
            !player.getStat("skill").PSdunshi &&
        ...
        initList: function (player) {
          ...
          for (var i of list) {
            ...
            for (var j of lib.character[i][3]) {
              ...
              if (info && info.indexOf("出牌", "受伤", "结束") != -1)
                skills.add(j);
            }
          }
  ...
  PSzhaoxiang: {
    ...
    skills: {
      PSfushi: {
        ...
        characterList: function () {
          ...
          lib.config.extension_PS武将_PScharacters =
            lib.config.extension_PS武将_PScharacters || [];
          lib.config.extension_PS武将_PSremoveCharacters =
            lib.config.extension_PS武将_PSremoveCharacters || [];
          lib.config.extension_PS武将_PSaddCharacter =
            lib.config.extension_PS武将_PSaddCharacter || [];
          lib.config.extension_PS武将_PSremoveCharacter =
            lib.config.extension_PS武将_PSremoveCharacter || [];
          ...
          function getAllCharacters(pack = false) {
            ...
            if (pack) {
              ...
              for (let a in lib.characterPack) {
                ...
                if (Object.prototype.hasOwnProperty.call(lib.characterPack, a))
                  allPack.push(a);
              }
              return allPack;
            } else {
              ...
              for (let b in lib.character) {
                if (Object.prototype.hasOwnProperty.call(lib.character, b))
                  allList.push(b);
              }
              return allList;
            }
          ...
          function getPackIdArray(uname = []) {
            ...
            for (let i of uname) {
              for (let j in lib.characterPack) {
                if (
                  Object.prototype.hasOwnProperty.call(lib.characterPack, j)
                ) {
                  if (
                    removeHTML(lib.translate[`${j}_character_config`]) === i ||
                    (lib.translate[`${j}_character_config`].includes(
                      "extension/"
                    ) &&
                      lib.translate[`${j}_character_config`].includes(i))
                  ) {
          ...
          let shed = getPackIdArray(
            lib.config.extension_PS武将_PSremoveCharacters
          ); //lib.config.extension_PS武将_PSremoveCharacters 移除的武将包名称数组
          pack.removeArray(shed); // 去除被移除的武将包数组
          for (let k of pack) {
            //提取武将包的武将id，放入list数组
            for (let l in lib.characterPack[k]) {
              if (Object.prototype.hasOwnProperty.call(lib.characterPack[k], l))
                list.push(l);
            }
          }
        ...
        chooseToFuHan: function () {
          ...
          var switchToAuto = function () {
            ...
            for (var i = 0; i < newList.length; i++) {
              if (lib.skill[newList[i]].ai && lib.skill[newList[i]].ai.combo)
          ...
          var chooseButton = function (list1, list2) {
            ...
            initBord.style.cssText =
              'width:586px;height:216px;top:33px;right:60px;positon:"absolute";';
            var skills = event.list2.flat(),
              num = 0;
            for (var i = 0; i < event.list1.length; i++) {
              var x =
                i * (104 - list1.length * 4) + (8 - list1.length) * 35 + 35;
              ...
              for (var j = 0; j < event.list2[i].length; j++) {
                ...
                if (
                  get.info(list2[i][j]).limited ||
                  get.info(list2[i][j]).juexingji
                )
            ...
            prompt.style.cssText =
              'width:100%;height:20px;left:0;bottom:0;text-align:center;font-family:"yuanli";font-size:20px;line-height:18px;color:#f1dfcc;filter: drop-shadow(1px 0 0 #664934) drop-shadow(-1px 0 0 #664934) drop-shadow(0 1px 0 #664934) drop-shadow(0 -1px 0 #664934);transform:translateY(220%);letter-spacing:3px;pointer-events:none;';
            prompt.textContent =
              "请选择" + get.cnNumber(event.total) + "个武将技能";
        ...
        content: function () {
          ...
          lib.character.PSzhaoxiang.dieAudios = [
            "ext:PS武将/audio/die/PSzhaoxiang2.mp3",
          ];
    ...
    translate: {
      ...
      "#ext:PS武将/audio/skill/PSfushi1":
        "逝者如斯，亘古长流，唯英烈之魂悬北斗而长存！",
      "#ext:PS武将/audio/skill/PSfushi2":
  ...
  PSlukang: {
    ...
    skills: {
      PSjueyan: {
        ...
        usable: 1,
        init: function (player, skill) {
          changeSkillAudio("PSjizhi", player.name, "rejizhi_lukang");
        },
        derivation: "PSjizhi",
        filter: function (event, player) {
          return (
  ...
  PSliru: {
    ...
    skills: {
      PSjuece: {
        ...
        filter: function (event, player) {
          if (
            !player.awakenedSkills.includes("xinfencheng") &&
            !player.awakenedSkills.includes("fencheng") &&
            !player.awakenedSkills.includes("dcfencheng")
          )
            return false;
          return player.hasCard(function (card) {
            return (
              (get.name(card) == "sha" && game.hasNature(card, "fire")) ||
              get.name(card) == "huogong"
            );
          }, "h");
        },
        filterCard: function (card) {
    ...
    skills: {
      PSkangkai: {
        ...
        content: function () {
          ...
          player.chooseTarget(
            1,
            "选择一名角色，令其将一张牌交给" + get.translation(trigger.target),
            true,
            function (card, player, target) {
      ...
      PShuashen: {
        ...
        filter: function (event, player, name) {
          ...
          return (
            ...
            player.storage.PShuashen.character.length > 0
          );
        },
        direct: true,
        content: function () {
          ...
          var choice = "更换技能";
          if (
            event.aiChoice == player.storage.PShuashen.current2 ||
            get.skillRank(event.aiChoice, event.cond) < 1 ||
            Math.random() < 0.3
          )
            choice = "制衡化身";
          if (player.isOnline2()) {
              ...
              function (cards, id) {
                var dialog = ui.create.dialog("是否发动【化身】？", [
                  ...
                  (item, type, position, noclick, node) =>
                    lib.skill.PShuashen.$createButton(
                      item,
                      type,
                      position,
                      noclick,
          ...
          //创建对话框
          event.dialog = ui.create.dialog(get.prompt("PShuashen"), [
            cards,
            (item, type, position, noclick, node) =>
              lib.skill.PShuashen.$createButton(
                item,
          ...
          if (event.logged && !event.isMine() && !event.isOnline())
            event._result = { control: "更换技能" }; // 在托管状态
          ...
          if (event.control == "制衡化身") {
            ...
            next.set("ai", function (button) {
              return player.storage.PShuashen.map[button.link].includes(
          ...
          var prompt =
            event.control == "制衡化身"
              ? "选择制衡至多两张化身"
              : `选择要获得的第${get.cnNumber(event.num, true)}个技能`;
          ...
          if (result.bool && event.control === "更换技能") {
            ...
            var list = player.storage.PShuashen.map[event.card]
              .slice(0)
              .filter((skill) => {
                if (event.num === 1) return true;
                ...
                dialog.buttons.forEach((btn) => {
                  ...
                  if (textNode.innerHTML.includes("✓</span>")) {
                    textNode.innerHTML = textNode.innerHTML
                      .replaceAll(
                        '<span style="color:#0a9b43; text-shadow: rgba(0, 0, 0, 0.6) 0 0 2px, rgba(0, 0, 0, 0.6) 0 0 5px, rgba(0, 0, 0, 0.6) 0 0 5px, rgba(0, 0, 0, 0.6) 0 0 5px, black 0 0 1px">',
                        ""
                      )
                      .replaceAll("✓</span>", "");
                  }
            ...
            if (dialog) {
              ...
              if (button) {
                ...
                textNode.innerHTML = textNode.innerHTML.replace(
                  skillName,
                  `<span style="color:#0a9b43; text-shadow: rgba(0, 0, 0, 0.6) 0 0 2px, rgba(0, 0, 0, 0.6) 0 0 5px, rgba(0, 0, 0, 0.6) 0 0 5px, rgba(0, 0, 0, 0.6) 0 0 5px, black 0 0 1px">${skillName}✓</span>`
                );
              }
            }
            if (
              !player.additionalSkills.PShuashen ||
              !player.additionalSkills.PShuashen.includes(link)
            ) {
              player.addAdditionalSkill("PShuashen", link, true);
              game.log(
                player,
                "获得了技能",
                "#g【" + get.translation(link) + "】"
              );
              player.popup(link);
              ...
              if (event.num <= 3) {
                ...
                if (!event.aiSkills.length) event.aiChoice = link;
                  ...
                  event.aiSkills.sort(function (a, b) {
                    return (
                      get.skillRank(b, event.cond) -
                      get.skillRank(a, event.cond)
                    );
                  });
        ...
        addHuashen: function (player) {
          ...
          for (let i = 0; i < _status.characterlist.length; i++) {
            ...
            if (
              ...
              name.indexOf("key_") == 0 ||
              name.indexOf("sp_key_") == 0 /* || get.is.double(name) */ ||
              lib.skill.PShuashen.banned.includes(name) ||
              player.storage.PShuashen.character.includes(name)
            )
              continue;
            let skills = lib.character[name][3].filter((skill) => {
              const categories = get.skillCategoriesOf(skill);
              return !categories.some((type) =>
                lib.skill.PShuashen.bannedType.includes(type)
              );
            });
            if (skills.length) {
          ...
          if (skills.length) {
            const skillstr = skills
              .map((i) => {
                ...
                if (
                  player.additionalSkills.PShuashen &&
                  player.additionalSkills.PShuashen.includes(i)
                )
            ...
            if (storage && storage.current2) {
              for (var i = 0; i < storage.current2.length; i++) {
                dialog.add(
                  '<div><div class="skill">【' +
                  get.translation(
                    lib.translate[storage.current2[i] + "_ab"] ||
                    get.translation(storage.current2[i]).slice(0, 2)
                  ) +
                  "】</div><div>" +
                  get.skillInfoTranslation(storage.current2[i], player) +
                  "</div></div>"
                );
              }
            }
      ...
      PSxinsheng: {
        ...
        content: function () {
          ...
          if (
            ...
            player
              .chooseBool(get.prompt2("PSxinsheng"))
      ...
      PSfenji: {
        ...
        filter: function (event, player) {
          if (event.name == "lose") {
            ...
            if (event.giver || event.getParent().name == "_yongjian_zengyu")
              return false;
            ...
            return game.hasPlayer(function (current) {
              return (
                current != event.discarder && event.getl(current).hs.length > 0
              );
            });
          }
          return false;
        ...
        content: function () {
          ...
          event.target = target;
          var num = player.getExpansions("PSbuqu").length
            ? player.getExpansions("PSbuqu").length + 2
            : 2;
          if (target.isIn())
            player
              .chooseBool(
                get.prompt("fenji", target),
                "失去1点体力，令该角色摸" + get.cnNumber(num) + "张牌"
              )
              .set("ai", function () {
                var evt = _status.event.getParent();
    ...
    skills: {
      ...
      PSluanshi: {
        ...
        content: function () {
          ...
          player.showCards(
            cards,
            "乱世：点数和为" + sum + "，" + get.translation(player) + str
          );
          game.delay(2);
          if (sum > 21) {
            player.recover();
          ...
          if (num == 0)
            player
              ...
              .set("ai", function (target) {
                var player = _status.event.player;
                var eff =
                  -get.attitude(player, target) - get.attitude(player, player);
                return eff;
              });
          else event.goto(3);
          ("step 2");
          ...
          player
            ...
            .set("ai", function (target) {
              ...
              var eff =
                -get.attitude(player, target) - get.attitude(player, player);
              var playerExpect =
                ((_status.event.num - 1) / 13) ** target.countCards("h");
              eff +=
                2 * playerExpect * get.attitude(player, player) +
                2 * (1 - playerExpect) * get.attitude(player, target) +
                1;
              return eff;
            })
            .set("num", num);
  ...
  PSnanhualaoxian: {
    ...
    intro: "由“一顿七只屑狐狸”设计",
    replace: ["PSnanhualaoxian", "PSshen_nanhualaoxian"],
    skills: {
      PSyufeng: {
        audio: "yufeng",
        enable: "phaseUse",
        ...
        content: function () {
          ...
          var func = function (player, id) {
            ...
            var choiceList = ui.create.dialog(
              "御风：请选择一" +
              (game.roundNumber > 1 ? "至" + get.cnNumber(num) : "") +
              "项"
            );
            choiceList.videoId = id;
            for (var i = 0; i < list.length; i++) {
      ...
      PSqiangzhi: {
        ...
        content: function () {
          ...
          game.addVideo("storage", player, [
            "PSqiangzhi_draw",
            player.storage.PSqiangzhi_draw,
          ]);
        ...
        subSkill: {
          draw: {
            ...
            filter: function (event, player) {
              return (
                get.type(event.card, "trick") == player.storage.PSqiangzhi_draw
              );
            },
            content: function () {
              player.draw();
  ...
  PSquansun: {
    ...
    skills: {
      ...
      PSnengwan: {
        ...
        forced: true,
        content: function () {
          var str = ["phaseDrawSkipped", "phaseDrawCancelled"].includes(
            event.triggername
          )
            ? "摸牌阶段"
            : "出牌阶段";
    ...
    skills: {
      PSzailaiyici: {
        ...
        filter: function (event, player) {
          ...
          if (["shan", "wuxie"].includes(get.name(event.card, event.player)))
            return false;
  ...
  PSguanyunchang: {
      ...
      PSweizhen: {
        mod: {
          ...
          cardUsable: function (card, player, num) {
            ...
            if (
              card.name == "sha" &&
              (get.color(card) == "red" || get.color(card) == "none")
            )
              return (num += info[3]);
          },
        },
        ...
        intro: {
          content: function (storage, player) {
            var info = lib.skill.PSweizhen.getInfo(player);
            return (
              '<div class="text center"><span class=thundertext>距离：' +
              info[0] +
              "</span>　<span class=firetext>伤害：" +
              info[1] +
              "</span><br><span class=greentext>目标：" +
              info[2] +
              "</span>　<span class=bluetext>次数：" +
              info[3] +
              "</span><br><span class=whitetext>强命：" +
              info[4] +
              "</span></div>"
            );
          },
        ...
        subSkill: {
            ...
            filter: function (event, player) {
              ...
              return (
                player.getHistory("useCard", function (evt) {
                  return (
                    get.name(evt.card) == "sha" && get.color(evt.card) == "red"
                  );
                }).length <= info[4]
              );
            },
          ...
          discard: {
            ...
            prompt2: function (event, player) {
              ...
              return (
                ...
                get.translation(event.player) +
                "一" +
                (info[4] > 1 ? "至" + get.cnNumber(info[4]) : "") +
                "张牌"
              );
            },
            filter: function (event, player) {
          ...
          chongzhu: {
            ...
            filterCard: (card, player) =>
              get.color(card) == "black" && player.canRecast(card),
            filter: function (card, player) {
              return (
    ...
    translate: {
      ...
      "#ext:PS武将/audio/skill/PSweizhen1":
        "狂徒！天下英雄闻我名无不丧胆，可惜我这青龙偃月刀，竟斩你这鼠辈的首级。",
      "#ext:PS武将/audio/skill/PSweizhen2": "我cnm。",
      "#ext:PS武将/audio/skill/PSweizhen3":
        "狂徒！我这青龙偃月刀竟斩你这鼠辈的首级。",
  ...
  PSzhangxuan: {
    ...
    intro: "由“看透一切的军师”设计",
    replace: ["PSzhangxuan", "PSfuzhijie", "PSjiesuanjie"],
    skills: {
      PStongli: {
        audio: "tongli",
        trigger: {
        ...
        filter: function (event, player) {
          if (
            !event.isFirstTarget ||
            (event.card.storage && event.card.storage.PStongli)
          )
            return false;
          var type = get.type(event.card);
          if (type != "basic" && type != "trick") return false;
  ...
  PScenhun: {
    ...
    skills: {
      PSlianhuo: {
        ...
        filter: function (event, player) {
          return (
            player.isLinked() && event.notLink() && event.hasNature("fire")
          );
        },
        content: function () {
    ...
    skills: {
      PSyuqi: {
        ...
        filter: function (event, player) {
          ...
          return (
            event.player.isIn() && get.distance(player, event.player) <= list[0]
          );
        },
        logTarget: "player",
        content: function () {
          ...
          next.set("list", [
            ...
            [
              ...
              "（至少一张" +
              (event.list[2] > 1
                ? "，至多" + get.cnNumber(event.list[2]) + "张"
                : "") +
              "）",
            ],
      ...
      PSshanshen: {
        ...
        content: function () {
          ...
          player
            ...
            .set(
              "prompt2",
              "令〖隅泣〗中的所有数字+2" + (event.goon ? "并回复1点体力" : "")
            )
            .set("ai", function () {
              return 0;
            });
  ...
  PSzhaoyun: {
    ...
    intro: "由“不想再说也许可以”设计",
    replace: ["PSzhaoyun", "PSqun_zhaoyun"],
    skills: {
      PShuwei: {
        audio: "ext:PS武将/audio/skill:2",
        trigger: {
  ...
  PScaochun: {
    ...
    intro: "由“你想去哪儿呢”设计",
    replace: ["PSshiniangongzhu", "PScaochun"],
    skills: {
      PSreshanjia: {
        locked: false,
        audio: "shanjia",
        ...
        content: function () {
                ...
                if (
                  [list[i]].includes(
                    get.type(
                      result.cards[j],
                      "trick",
                      result.cards[j].original == "h" ? player : false
                    )
                  )
                ) {
                  bool[i] = true;
          ...
          if (event.bool[1]) {
            ...
            if (list.length) {
              player
                .chooseButton(["是否视为使用一张普通锦囊牌？", [list, "vcard"]])
      ...
      PSxiaorui: {
        ...
        filterTarget: true,
        prompt:
          "失去一点体力并获得一名角色两张牌，再对其造成一点伤害，若其中有装备牌，你回复一点体力",
        content: function () {
          "step 0";
          player.loseHp(1);
          ("step 1");
  ...
  PScaoshuang: {
    ...
    skills: {
      PStuogu: {
        ...
        filter: function (event, player) {
          return (
            event.player.getStockSkills("鸡", "你").filter(function (skill) {
        ...
        content: function () {
          ...
          player
            ...
            .set(
              "prompt",
              "选择获得" + get.translation(trigger.player) + "的一个技能"
            )
            .set("forceDie", true)
            .set("ai", function () {
    ...
    skills: {
      PSmiewu: {
          ...
          dialog: function (event, player) {
            ...
            for (var i = 0; i < lib.inpile.length; i++) {
              ...
              if (name == "sha") {
                ...
                for (var j of lib.inpile_nature) {
                  if (
                    event.filterCard({ name: name, nature: j }, player, event)
                  )
                    list.push(["基本", "", "sha", j]);
                }
              } else if (
        ...
        hiddenCard: function (player, name) {
          ...
          return (
            (type == "basic" || type == "trick" || type == "delay") &&
            !player.hasSkill("PSmiewu_failure") &&
        ...
        ai: {
          ...
          respondShan: true,
          skillTagFilter: function (player) {
            if (!player.countCards("hse") || player.hasSkill("PSmiewu_failure"))
              return false;
          },
          order: 1,
          result: {
      ...
      PSkefu: {
        ...
        subSkill: {
          ...
          recover: {
            ...
            filter: function (event, player) {
              return (
                event.card && event.card.storage && event.card.storage.PSkefu
              );
            },
            content: function () {
  ...
  PShs_zhonghui: {
    ...
    intro: "由“sc蓝晨跃”设计",
    replace: ["PShs_zhonghui", "PSzhonghui"],
    skills: {
      PSmn_quanji: {
        ...
        content: function () {
          ...
          if (event.count > 0)
            player
              .chooseBool(get.prompt("PSmn_quanji"), "摸两张牌")
              .set("frequentSkill", "PSmn_quanji");
          else event.finish();
          ("step 3");
          if (result.bool) {
        ...
        subSkill: {
          phase: {
            ...
            check: function (card) {
              ...
              if (
                !player.getExpansions("quanji").length ||
                num -
                ui.selected.cards.length -
                (player.getExpansions("quanji").length +
                  ui.selected.cards.length) >
  ...
  PSboss_lvbu1: {
    ...
    intro: "由“神鬼高达”设计",
    replace: ["PSboss_lvbu1", "PSboss_lvbu2", "PSboss_lvbu3", "PSboss_lvbu4"],
    skills: {
      PSjingjia1: {
        derivation: [
          "wushuangfangtianji",
      ...
      PSaozhan1: {
        ...
        subSkill: {
          ...
          zuoji: {
            ...
            filter: function (event, player) {
              return (
                player.getEquip(3) || player.getEquip(4) || player.getEquip(6)
              );
            },
            content: function () {
              trigger.num++;
      ...
      PSbaguan1: {
        ...
        subSkill: {
              ...
              for (var name of list) {
                if (!lib.inpile.includes(name)) {
                  ...
                  ui.cardPile.insertBefore(
                    ...
                    get.rand(0, ui.cardPile.childNodes.length)
                    ]
                  );
                  game.log(name, "加入了牌堆");
          ...
          phase: {
            ...
            filter: function (event, player) {
              return (
                event.player != player && _status.currentPhase.next != player
              );
            },
            priority: -1,
            forced: true,
          ...
          bianshen: {
            ...
            content: function () {
              ...
              if (result.control == "暴怒战神") {
                lib.skill.PSbaguan1.reinitCharacter(
                  player,
                  "PSboss_lvbu1",
                  "PSboss_lvbu2",
                  [6, 6]
    ...
    skills: {
        ...
        content: function () {
          ...
          player
            .chooseToDiscard(
              "hes",
              2,
              "神威：弃置两张牌并弃置自己判定区的所有牌",
              true
            )
            .set("logSkill", "PSshenwei2").ai = function (card) {
      ...
      PSbaguan2: {
        ...
        content: function () {
          ...
          ("step 2");
          const name = player.name1.startsWith("PSboss_lvbu")
            ? player.name1
            : player.name2 && player.name2.startsWith("PSboss_lvbu")
              ? player.name2
              : player.name1;
          if (result.control == "暴怒战神") {
    ...
    skills: {
      ...
      PSjiwu3: {
        ...
        check: function (card) {
          if (
            get.position(card) == "e" &&
            _status.event.player.hasSkill("decadexuanfeng")
          )
        ...
        subSkill: {
          ...
          audio: {
            ...
            filter: function (event, player) {
              return (
                event.skill == "qiangxix" ||
                event.skill == "rewansha" ||
                event.skill == "retieji" ||
                event.skill == "decadexuanfeng" ||
                event.skill == "wushuang"
              );
            },
            content: function () {
        ...
        content: function () {
          ...
          player
            .chooseTarget(
              1,
              lib.filter.notMe,
              "神愤：是否令一名角色失去一点体力？或点击“取消”回复一点体力",
              false
            )
            .set("ai", function (target) {
              var att = -get.attitude(_status.event.player, target);
            ...
            player
              .chooseControl(list, function () {
                ...
                if (!player.getStat().skill.PSmn_qiangxix) {
                  if (
                    player.hasSkill("PSmn_qiangxi") &&
                    player.getEquip(1) &&
                    list.includes("decadexuanfeng")
                  )
                    return "decadexuanfeng";
                  if (
                    list.includes("rewansha") ||
                    list.includes("PSmn_qiangxi")
                  ) {
                    var players = game.filterPlayer();
                    for (var i = 0; i < players.length; i++) {
                      if (
      ...
      PSmn_qiangxi: {
        ...
        content: function () {
          ...
          player
            .chooseToDiscard(
              "he",
              get.prompt("PSmn_qiangxi", trigger.player),
              "弃置一张装备牌并令此伤害+1",
              function (card) {
            ...
            filter: function (event, player) {
              return game.hasPlayer(function (target) {
                return (
                  ...
                  !target.hasSkill("PSmn_qiangxi_used")
                );
              });
            },
            filterTarget: function (card, player, target) {
            ...
            ai: {
              ...
              result: {
                target: function (player, target) {
                  //主公内奸矜持，其他身份当疯狗
                  var bool =
                    lib.translate[player.identity] == "主" ||
                    lib.translate[player.identity] == "内" ||
                    (get.mode() == "identity" &&
                      player.hasSkill("olzaowang2") &&
                      lib.translate[player.identity] != "nei");
                  if (bool && target.hp - player.hp > 1) return 0;
                  if (
                    !bool &&
                    player.hp < 2 &&
                    !player.countCards("hs", { name: ["tao", "jiu"] })
                  )
                    return 0;
                  return -1;
                },
    ...
    skills: {
      PStaoluan: {
          ...
          dialog: function (event, player) {
            ...
            for (var i = 0; i < lib.inpile.length; i++) {
              ...
              if (
                ...
                player.storage.PStaoluan.includes(name)
              )
                continue;
              if (name == "sha") {
                if (event.filterCard({ name: name }, player, event))
        ...
        hiddenCard: function (player, name) {
          ...
          return (
            (type == "basic" || type == "trick") && player.countCards("she") > 0
          );
        },
        ai: {
          fireAttack: true,
      ...
      PSxianfu: {
        ...
        content: function () {
          ...
          player
            .chooseTarget(
              "请选择【先辅】的目标",
              lib.skill.PSxianfu_info,
              true,
              function (card, player, target) {
                return (
                  target != player &&
                  (!player.storage.PSxianfu_target ||
                    !player.storage.PSxianfu_target.includes(target))
                );
              }
            )
            .set("ai", function (target) {
        ...
        subSkill: {
          target: {
            ...
            filter: function (event, player) {
              if (
                event.player.isDead() ||
                !player.storage.PSxianfu_target ||
                !player.storage.PSxianfu_target.includes(event.player) ||
                event.num <= 0
              )
                return false;
            ...
            onremove: function (player) {
              ...
              game.countPlayer(function (current) {
                if (
                  player.storage.PSxianfu_target.includes(current) &&
                  current.storage.PSxianfu_mark
                ) {
          ...
          mark: {
            ...
            intro: {
              name: "先辅",
              content:
                "当你受到伤害/回复体力/摸牌/弃牌后，$受到等量的伤害/回复等量的体力/摸等量的牌/弃置等量的牌",
            },
            sub: true,
          },
          lose: {
      ...
      PSchouce: {
        ...
        content: function () {
          ...
          if (event.color == "black") {
            ...
            if (
              player.storage.PSxianfu_target &&
              player.storage.PSxianfu_target.length
            ) {
              next.set(
                "prompt2",
                "（若目标为" +
                get.translation(player.storage.PSxianfu_target) +
                "则改为摸两张牌）"
              );
            }
    ...
    skills: {
      ...
      PSzengxi: {
        ...
        content: function () {
          ...
          player
            .chooseTarget(
              "赠玺：选择一名其他角色，你交给他装备区所有牌，他交给你一半手牌（向下取整）",
              1,
              true
            )
            .set("filterTarget", function (card, player, target) {
            ...
            .set("ai", function (target) {
              var player = _status.event.player;
              if (
                player.countCards("e") > target.countCards("h") &&
                get.attitude(player, target) > 0
              )
                return 2;
              if (
    ...
    skills: {
      PShengce: {
        ...
        content: function () {
          ...
          ("step 2");
          player
            .chooseTarget("衡策：对一名角色造成两点伤害", 1, false)
            .set("ai", function (target) {
              var player = _status.event.player;
          ...
          ("step 4");
          player
            .chooseTarget("衡策：令一名角色摸四张牌", 1, false)
            .set("ai", function (target) {
              var player = _status.event.player;
              if (get.attitude(player, target) > 0) return 5;
              return 0;
          ...
          ("step 12");
          player
            .chooseTarget("衡策：令一名角色增加两点体力上限", 1, false)
            .set("ai", function (target) {
              var player = _status.event.player;
              if (get.attitude(player, target) > 0) return 5;
              return 0;
      ...
      PSxiangong: {
        ...
        content: function () {
          ...
          if (result.bool == true) {
            ...
            target
              .addToExpansion(player, "give", cards)
              .gaintag.add("PSxiangong_cards");
        ...
        subSkill: {
          ...
          damage: {
            ...
            filter: function (event, player) {
              ...
              return game.hasPlayer((current) => {
                return (
                  current.getExpansions("PSxiangong_cards").length >= event.num
                );
              });
            },
            direct: true,
            content: function () {
              ...
              player
                ...
                .set("filterTarget", function (card, player, target) {
                  return (
                    target.getExpansions("PSxiangong_cards").length >=
                    trigger.num
                  );
                })
                .set("ai", function (target) {
              ...
              if (result.targets) {
                ...
                target.chooseButton(
                  [
                    "献贡：请选择你的贡品",
                    target.getExpansions("PSxiangong_cards"),
                  ],
                  trigger.num,
                  true
                );
          ...
          if (_status.characterlist) {
            ...
            list = get.charactersOL(function (i) {
              return (
                ...
                !lib.skill.PSfushi.characterList().includes(i)
              );
            });
          } else {
            list = get.gainableCharacters(function (info, i) {
              return (
                info[1] == "wu" && lib.skill.PSfushi.characterList().includes(i)
              );
            });
          }
          var players = game.players.concat(game.dead);
      ...
      PStanbei: {
        ...
        filterTarget: function (card, player, target) {
          return (
            ...
            !target.hasSkill("tanbei_effect2")
          );
        },
        content: function () {
          "step 0";
      ...
      PSsidao: {
        ...
        content: function () {
          ...
          next.set(
            ...
            game.filterPlayer(function (current) {
              return (
                targets.includes(current) && trigger.targets.includes(current)
              );
            })
          );
          next.set(
            "openskilldialog",
            get.prompt("PSsidao") + lib.translate.PSsidao_info
          );
    ...
    skills: {
      PSdingjun: {
        ...
        filter: function (event, player) {
          ...
          return (
            event.targets.length == 1 &&
            get.suit(event.card) != "none" &&
            get.number(event.card) != null
          );
        },
        preHidden: true,
        content: function () {
      ...
      PShuishi: {
        ...
        content: function () {
          ...
          player
            ...
            .set("callback", lib.skill.PShuishi.callback).judge2 = function (
              result
            ) {
          ...
          if (cards.length)
            player
              .chooseTarget(
                "将" + get.translation(cards) + "交给一名角色",
                true
              )
              .set("ai", function (target) {
                var player = _status.event.player;
        ...
        callback: function () {
          ...
          if (event.getParent().result.bool) {
            ...
            player
              .chooseBool("是否继续发动【慧识】？")
              .set("frequentSkill", "PShuishi");
      ...
      PSluoshen: {
        ...
        content: function () {
          ...
          trigger.player
            ...
            .set(
              "prompt",
              "洛神：请弃置两张花色不同的黑色牌，否则展示牌堆底三张牌，将其中的黑色牌置于" +
              get.translation(player) +
              "武将牌上"
            )
        ...
        subSkill: {
          use: {
            ...
            content: function () {
              "step 0";
              player.chooseBool(
                "是否将" + get.translation(trigger.cards) + "置于牌堆底？"
              );
              ("step 1");
              if (result.bool) {
        ...
        subSkill: {
            ...
            filter: function (event, player) {
              return (
                ...
                !player.hasSkill("PSshixin_blocker")
              );
            },
            content: function () {
              player.recover();
      ...
      PShunwu: {
        ...
        content: function () {
          ...
          player.judge(function (card) {
            if (
              ["sha", "juedou", "nanman", "wanjian", "huogong"].includes(
                card.name
              )
            )
              return 10;
    ...
    skills: {
                ...
                game.broadcastAll(
                  function (player, storage) {
                    if (player.marks.PSxingtu)
                      player.marks.PSxingtu.firstChild.innerHTML =
                        (game.HasExtension("十周年UI") ? "X=" : "") + storage;
                  },
                  player,
      ...
      PSjuezhi: {
        ...
        prompt: function () {
          var player = _status.event.player;
          if (
            player.hasSkill("PSxingtu") &&
            !player.hasSkill("PSxingtu_disable")
          )
            return (
              "〖行图〗②未失效，是否继续发动技能？" +
              lib.translate.PSjuezhi_info
            );
        ...
        intro: {
          content: function (storage, player, skill) {
            if (player.storage.PSlongnu == true)
              return "锁定技，出牌阶段开始时，你失去1点体力并摸一张牌，然后本阶段内你视为拥有技能〖怒斩〗，且你的红色手牌均视为火【杀】且无距离限制。";
        ...
        content: function () {
          ...
          if (result.bool) {
            ...
            player
              ...
              .set(
                "prompt",
                "三陈：弃置" + get.translation(target) + "花色不同的牌"
              )
              .set("complexCard", true);
          }
        },
        ...
        content: function () {
          ...
          if (map && map.skills && map.skills.length) {
            for (var i of map.skills) {
              ...
              player.addTempSkill(
                i,
                event.triggername == "damageEnd" ? "damageAfter" : "phaseJieshu"
              );
            }
          }
          game.broadcastAll(function (list) {
            ...
            content: function () {
              ...
              if (result.bool) {
                ...
                for (var i = 0; i < _status.characterlist.length; i++) {
                  ...
                  var skills2 = lib.character[name][3];
                  for (var j = 0; j < skills2.length; j++) {
                    if (player.storage.PSsb_pingjian.includes(skills2[j]))
                      continue;
                    if (
                      (skills.includes(skills2[j]) ||
                        lib.skill.PSsb_pingjian.phaseUse_special.includes(
                          skills2[j]
                        )) &&
                      lib.skill.PSfushi.characterList().includes(name)
                    ) {
          ...
          temp: {
            ...
            filter: function (event, player) {
              ...
              if (player.storage.PSsb_pingjian_temp.includes(event.skill))
                return true;
              if (
                player.storage.PSsb_pingjian_temp.includes(info.sourceSkill) ||
                player.storage.PSsb_pingjian_temp.includes(info.group)
              )
                return true;
              if (
                Array.isArray(info.group) &&
                (info.group.includes(player.storage.PSsb_pingjian_temp[0]) ||
                  info.group.includes(player.storage.PSsb_pingjian_temp[1]))
              )
                return true;
              return false;
            },
          ...
          use: {
            ...
            content: function () {
              ...
              if (event.index == 1) {
                ...
                if (
                  ui.cardPile.childElementCount == 1 ||
                  player == event.target
                ) {
                  player.draw();
                } else {
          ...
          tag: {
            ...
            filter: function (event, player) {
              ...
              return (
                ...
                event.getParent(2).name == "PSwengua_use"
              );
            },
            content: function () {
      ...
      PSfuzhu: {
        ...
        subSkill: {
          achieve: {
            ...
            filter: function (event, player) {
              return (
                event.getParent(5).name == "PSfuzhu" &&
                event.player == player.storage.PSfuzhu_target
              );
            },
            content: function () {
              game.log(player, "成功完成使命");
        ...
        content: function () {
          ...
          if (event.skills.length > 1) {
            player
              ...
              .set(
                "prompt",
                "请选择禁用" + get.translation(trigger.player) + "的一个技能"
              )
              .set("ai", function () {
          ...
          trigger.player.addTempSkill("PSduorui_disable", {
            player: "phaseAfter",
          });
  ...
  PSguanyu: {
    ...
    intro: "由“一顿七只屑狐狸”设计",
    replace: ["PSguanyunchang", "PSguanyu"],
    skills: {
      PSwusheng: {
        mod: {
          aiOrder: function (player, card, num) {
      ...
      PSyijue: {
        ...
        content: function () {
          ...
          if (map.bool) {
            ...
            player.popup(
              get.translation(event.suit + 2 || "任意花色") +
              "<br/>" +
              get.translation(event.type || "任意类型"),
              "thunder"
            );
          }
          ("step 3");
          if (
            !target.hasCard((card) => {
              return (
                get.suit(card) == (event.suit || get.suit(card)) &&
                get.type2(card) == (event.type || get.type2(card))
              );
            }, "he")
          )
            ...
            target
              ...
              .set(
                "prompt",
                "交给" + get.translation(player) + "一张牌，否则获得负面效果"
              );
          ("step 4");
          if (result.bool && result.cards) {
            ...
            if (target.hp < target.maxHp) {
              player.chooseBool(
                "是否让" + get.translation(target) + "回复一点体力？"
  ...
  PSshen_zhangfei: {
    ...
    skills: {
      PSshencai: {
        ...
        contentx: function () {
          ...
          for (var i in lib.skill.PSshencai.filterx) {
            if (str.indexOf(lib.skill.PSshencai.filterx[i]) != -1)
      ...
      PSxunshi: {
        ...
        content: function () {
          if (
            player.countMark("PSshencai") < 4 &&
            player.hasSkill("PSshencai", null, null, false)
          )
    ...
    skills: {
      PStaoyuan: {
        ...
        filter: function (event, player) {
          return (
            event.num < 0 &&
            game.hasPlayer((current) => current.group === "shu")
          );
        },
        content: function () {
          "step 0";
          ...
          ("step 2");
          player
            .chooseTarget("桃园：选择一名角色，让其挑选武将牌")
            .set("ai", function (target) {
              var player = _status.event.player;
              var att =
                get.attitude(player, target) /
  ...
  PSshen_dengai: {
    ...
    skills: {
      PStuoyu: {
        ...
        mod: {
          ...
          cardDiscardable: function (card, _, name) {
            if (
              name == "phaseDiscard" &&
              get.info("PStuoyu").cardIsIgnored(card)
            )
              return false;
          },
        },
      ...
      PSxianjin: {
        ...
        filter: function (event, player, name) {
          return (
            name === "roundStart" || player.countMark("dcxianjin") % 2 == 0
          );
        },
        content: function () {
          "step 0";
      ...
      PSqijing: {
        ...
        filter: function (event, player) {
          return (
            !event.skill &&
            event.player.next == _status.roundStart &&
            player.getStorage("dctuoyu").length == 3
          );
        },
        forced: true,
        juexingji: true,
        ...
        content() {
          ...
          lib.character.PSshen_dengai.dieAudios = [
            "ext:PS武将/audio/die/PSshen_dengai2.mp3",
          ];
    ...
    translate: {
      ...
      "#ext:PS武将/audio/skill/PSxianjin_PSshen_dengai22":
        "鸥鹭擒蚌，必以利喙击其坚甲，而后得脍。",
      "#ext:PS武将/audio/skill/PSqijin_PSshen_dengai21":
        "世人皆寻已有之平路，独吾愿为开路之先锋。",
      "#ext:PS武将/audio/skill/PSqijin_PSshen_dengai22":
        "纵尔匿离天之穹、九幽之渊，吾亦擒汝首而还。",
      "#ext:PS武将/audio/skill/cuixin_PSshen_dengai21":
        "缚咖于颈，镇岳于魂，教汝死灰不燃、万劫不复！",
      "#ext:PS武将/audio/skill/cuixin_PSshen_dengai22":
        "上军争功，夺三家之士气，击天下之雄兵。",
      "#ext:PS武将/audio/die/PSshen_dengai2:die":
        "巨灵不屑顾，奈何反被宵小伤。",
    },
    rank: rank.epic,
  },
      ...
      PSlingce: {
        ...
        content() {
          ...
          if (player.storage.PSdinghan == true)
            ...
            player
              .chooseCardButton(cards, "选择获得的牌名不同的牌", [1, Infinity])
              .set("filterButton", function (button) {
        ...
        filter: (event, player) => {
          return (
            player.getAllHistory("useSkill", (evt) =>
              ["PStianzuo", "PStianzuo_use"].includes(evt.skill)
            ).length >= 3
          );
        },
        content: () => {
  ...
  PShaozhao: {
    ...
    skills: {
      PSzhengu: {
        ...
        filter: function (event, player) {
          return (
            !player.hasSkill("PSzhengu_mark") ||
            game.hasPlayer(
              (current) =>
                current != player && !current.hasSkill("PSzhengu_mark")
            )
          );
        },
        async cost(event, trigger, player) {
        ...
        async content(event, trigger, player) {
          ...
          if (player.hasSkill("PSzhengu_mark")) {
            ...
            ({ control } = await player
              ...
              .set(
                "prompt",
                `令你或${get.translation(target)}变成“高达”：手牌数始终保持与另一方一致，直到“高达”的回合结束时`
              )
              .set("target", target)
              .set("ai", function () {
  ...
  PSxuyou: {
    ...
    skills: {
      PSchenglve: {
        ...
        content: function () {
          ...
          if (result.bool) {
            ...
            for (var i = 0; i < result.cards.length; i++) {
              player.storage.PSchenglve_effect.add(
                get.suit(result.cards[i], player)
              );
            }
      ...
      PSshicai: {
        ...
        filter: function (event, player, name) {
          if (
            name == "useCardToTargeted" &&
            ("equip" != get.type(event.card) || event.player != player)
          )
            return false;
          if (
            name == "useCardAfter" &&
          ...
          for (var i = 0; i < history.length; i++) {
            if (
              history[i] != evt &&
              get.name(history[i].card) == get.name(event.card)
            )
              return false;
        ...
        check: function (event, player) {
          if (get.type(event.card) == "equip") {
            ...
            return (
              player.countCards("h", function (card) {
                return (
                  get.subtype(card) == subtype &&
                  player.getUseValue(card) >= eff1
                );
              }) > 0
            );
          }
    ...
    skills: {
      PSyuheng: {
          ...
          for (var i of list) {
            skills.addArray(
              (lib.character[i][3] || []).filter(function (skill) {
                var info = get.info(skill);
                return (
                  info && !info.zhuSkill && !info.hiddenSkill && !info.charlotte
                );
              })
            );
          }
        ...
        content: function () {
          ...
          if (result.bool) {
            ...
            var skills = list.randomGets(
              Math.min(list.length, result.cards.length)
            );
            if (
              player.additionalSkills.PSyuheng &&
              player.additionalSkills.PSyuheng.length
            ) {
            ...
            content: function () {
              ...
              if (result.bool) {
                ...
                game.log(
                  player,
                  "失去了以下技能：",
                  "#g" + get.translation(links)
                );
                player.draw(links.length);
                player.removeSkills(links);
                player.additionalSkills.PSyuheng.length
                  ? player.markSkill("PSyuheng")
                  : player.unmarkSkill("PSyuheng");
              }
            },
            sub: true,
          },
      ...
      PSdili: {
        ...
        filter: function (event, player, name) {
          ...
          return (
            player.additionalSkills.PSyuheng &&
            player.additionalSkills.PSyuheng.length > player.maxHp
          );
        },
        forced: true,
        skillAnimation: true,
      ...
      PSgongxin: {
        ...
        content: function () {
          ...
          if (result.bool) {
            ...
            player.storage.PSbolan -= Math.min(
              result.links.length,
              player.storage.PSbolan
            );
            player.storage.PSbolan
              ? player.markSkill("PSbolan")
              : player.unmarkSkill("PSbolan");
          }
      ...
      PSpaiyi: {
        ...
        chooseButton: {
          dialog: function (event, player) {
            return ui.create.dialog(
              "排异",
              player.getExpansions("PSquanshu"),
              "hidden"
            );
          },
          backup: function (links, player) {
              ...
              ai: {
                ...
                result: {
                  target: function (player, target) {
                    ...
                    if (
                      player.getExpansions("PSquanshu").length <= 1 ||
        ...
        contentx: function () {
          ...
          target
            .chooseTarget(
              false,
              [1, max],
              "对至多" + get.cnNumber(max) + "名角色各造成1点伤害"
            )
            .set("ai", function (target) {
              var player = _status.event.player;
              return get.damageEffect(target, player, player);
    ...
    skills: {
        ...
        filter: function (event, player, name) {
          if (lib.config.extension_PS武将_PS_pingzeTip == true) {
            let hs = player.getCards(
              "h",
              (card) =>
                !card.hasGaintag("PSshixian_ping") &&
                !card.hasGaintag("PSshixian_ze")
            );
            let hs1 = player.getCards(
              "h",
              (card) =>
                hs.includes(card) &&
                getPingZe(get.translation(card.name)) == "平"
            );
      ...
      PSshixian: {
        ...
        filter: function (event, player) {
          ...
          player.marks.PSshixian.firstChild.innerHTML = getPingZe(
        ...
        prompt2: function (event, player) {
          if (lib.skill.PSshixian.filterx(event))
            return (
              "摸一张牌并令" + get.translation(event.card) + "额外结算一次？"
            );
          return "摸一张牌。";
        },
        check: function (event, player) {
          ...
          aiOrder: function (player, card, num) {
            if (typeof card == "object" && !get.tag(card, "norepeat")) {
              ...
              if (history.length > 0) {
                ...
                if (
                  getPingZe(get.translation(card.name)) !==
                  getPingZe(get.translation(cardx.name))
                )
                  return num + 20;
              }
            }
  ...
  PSxie_sunquan: {
    ...
    skills: {
      PSzongheng: {
        ...
        position: "h",
        filter: function (event, player) {
          const skills = player.getHistory(
            "useSkill",
            (evt) => evt.skill == "PSzongheng"
          );
        ...
        filterCard: function (card, player, event) {
          if (ui.selected.cards.length > 1) {
            var suit = get.suit(card);
            if (
              get.suit(ui.selected.cards[0]) != get.suit(ui.selected.cards[1])
            ) {
              for (var i of ui.selected.cards) {
        ...
        subSkill: {
          draw: {
            ...
            filter: function (event, player) {
              if (
                event.getParent(2).skill != "PSzongheng" &&
    ...
    skills: {
      PSliangzhu: {
        ...
        content: function () {
          ...
          player
            .chooseToDiscard(
              `是否弃置一张牌令${get.translation(trigger.player)}摸两张牌`,
              "he",
              false,
              1
            )
        ...
        filter: function (event, player) {
          ...
          return game.hasPlayer(function (current) {
            return (
              ...
              player.storage.PSliangzhu.includes(current) &&
              current.isDamaged()
            );
          });
        },
    ...
    skills: {
      ...
      PSxingluan: {
        ...
        filter: function (event, player) {
          ...
          return (
            player.countCards("he") >=
            lib.skill.PSxingluan.getNum(event, player)
          );
        },
        prompt2: function (event, player) {
    ...
    skills: {
        ...
        subSkill: {
          use: {
            ...
            filter: function (event, player) {
              return (
                event.skill &&
                get.info(event.skill).equipSkill &&
                ["muniu_skill", "bagua_skill", "zhuge_skill"].includes(
                  event.skill
                )
              );
            },
      ...
      PShuoji: {
        ...
        content: function () {
          var card = get.cardPile2(function (card) {
            return (
              get.translation(card).includes("火") ||
              lib.translate[card.name + "_info"].includes("火")
            );
          });
          if (card) {
            player.gain(card, "gain2");
      ...
      PSpingnan: {
        ...
        content: function () {
          ...
          const filterTarget = targets.filter(
            (current) =>
              current.isAlive() &&
              current.isIn() &&
              current !== player &&
    ...
    skills: {
      PSduodao: {
        ...
        content: function () {
          ...
          player.draw();
          var target =
            event.triggername === "useCardToTargeted"
              ? trigger.player
              : trigger.source;
          ...
          if (
            !result.bool ||
            (result.bool && result.cards[0].original !== "h")
          ) {
            player.draw();
          }
        },
  ...
  PSliubei: {
    ...
    intro: "由“九个芒果”设计",
    replace: ["PSliubei", "PSmeng_liubei"],
    skills: {
      PSrende: {
        audio: "ext:PS武将/audio/skill:2",
        enable: "phaseUse",
        ...
        content: function () {
          ...
          targets[0]
            .chooseCard(
              "h",
              true,
              [1, targets[0].countCards("h")],
              "选择交给" + get.translation(targets[1]) + "至少一张牌"
            )
            .set("ai", (card) => {
    ...
    skills: {
      PSjushou: {
        ...
        content() {
          ...
          if (list.length) {
            player
              .chooseButton(["据守：视为使用一张锦囊牌", [list, "vcard"]])
              .set("ai", function (button) {
                var player = _status.event.player;
                var card = {
                  name: button.link[2],
            ...
            content() {
              ...
              player
                .chooseToDiscard(
                  "he",
                  1,
                  get.prompt("PSjushou_turnOver"),
                  "当你翻面时，你可以弃置一张牌，然后选择回血或者移动场上的牌"
                )
                .set("ai", function (card) {
    ...
    skills: {
      PSrepaoxiao: {
        ...
        filter(event, player, name) {
          ...
          if (name === "respond")
            return (
              player.getHistory("respond", (evt) => evt.card.name === "sha")
                .length === 1
            );
          return (
      ...
      PSliyong: {
        ...
        content: function () {
          switch (event.triggername) {
            ...
            case "useCardToPlayered":
              ...
              if (
                player.hasHistory("custom", (evt) => evt.PSliyong_effect) &&
                trigger.isFirstTarget
              ) {
        ...
        subSkill: {
          draw: {
            ...
            filter: function (event, player, name) {
              return (
                player.storage.PSliyong && player.storage.PSliyong.length > 0
              );
            },
            forced: true,
            content: function () {
      ...
      PSrelianying: {
        ...
        subSkill: {
            ...
            content: function () {
              ...
              player
                .chooseBool(
                  `是否弃置${_status.currentPhase === player ? "" : get.translation(_status.currentPhase) + "的"}一张手牌，然后摸一张牌`
                )
                .set("ai", function () {
                  return 1;
                });
      ...
      PSxionghuo: {
        ...
        subSkill: {
          ...
          damage: {
            ...
            filter: function (event, player) {
              return (
                event.player.countMark("PSxionghuo") > 0 &&
                event.player != player
              );
            },
            forced: true,
          ...
          disable: {
            mod: {
              playerEnabled: function (card, player, target) {
                if (
                  card.name == "sha" &&
                  player.getStorage("PSxionghuo_disable").includes(target)
                )
                  return false;
              },
            },
      ...
      PSshajue: {
        ...
        content: function () {
          ...
          if (
            ...
            get.position(trigger.parent.cards[0], true) == "o"
          ) {
  ...
  PSlifeng: {
    ...
    skills: {
      PStunchu: {
        ...
        content: function () {
          ...
          if (hs.length) {
            if (hs.length <= 2) event._result = { bool: true, cards: hs };
              ...
              player
                .chooseCard("h", 2, true, "选择两张手牌作为“粮”")
                .set("ai", function (card) {
                  return 5 - get.value(card);
                });
          }
        ...
        subSkill: {
          ...
          choose: {
            ...
            filter: function (event, player) {
              return (
                player.getExpansions("PStunchu").length <=
                Math.min(player.maxHp, 3) && player.countCards("h")
              );
            },
            content: function () {
              "step 0";
              ...
              player
                .chooseCard(
                  ...
                  [1, num],
                  false,
                  `屯储：是否选择至多${get.cnNumber(num)}张手牌作为“粮”`
                )
                .set("ai", function (card) {
                  return 5 - get.value(card);
                });
      ...
      PSshuliang: {
        ...
        filter: function (event, player) {
          return (
            player.getExpansions("PStunchu").length > 0 &&
        ...
        content: function () {
          ...
          player
            .chooseCardButton(
              get.prompt("PSshuliang", trigger.player),
              player.getExpansions("PStunchu")
            )
            .set("ai", function () {
    ...
    skills: {
      PSqingbei: {
        ...
        content: function () {
          ...
          var next = player.chooseButton(
            [
              '###擎北：是否选择任意种花色？###<div class="text center">本轮你使用牌后，若此牌的花色与你以此法选择花色均不相同，你摸X张牌，否则你须弃置X张牌（X为你本轮以此法选择的花色数）</div>',
        ...
        subSkill: {
          effect: {
            ...
            content: function () {
              ...
              if (player.getStorage("PSqingbei_effect").includes(suit))
                player.chooseToDiscard(
                  "hes",
                  true,
                  player.getStorage("PSqingbei_effect").length
                );
            ...
            intro: {
              content: (storage) =>
                `本轮内使用非${get.translation(storage)}花色的牌后，摸${get.cnNumber(storage.length)}张牌；<br>使用${get.translation(storage)}花色的牌后，弃置${get.cnNumber(storage.length)}张牌。`,
            },
            ai: {
              effect: {
                player: function (card, player, target) {
  ...
  PSben_sunben: {
    ...
    skills: {
      ...
      PSduwu: {
        ...
        filter: function (event, player) {
          return (
            !player.storage.PSduwu &&
            !player.getHistory("useSkill", (evt) => evt.skill === "PShunzi")
              .length
          );
        },
        skillAnimation: true,
      ...
      PSchoushang: {
        ...
        filter: function (event, player) {
          return (
            !player.storage.PSchoushang &&
            !player.getHistory("useSkill", (evt) => evt.skill === "PSduwu")
              .length
          );
        },
        skillAnimation: true,
      ...
      PShuaiji: {
        ...
        filter: function (event, player) {
          return (
            !player.storage.PShuaiji &&
            !player.getHistory("useSkill", (evt) => evt.skill === "PSchoushang")
              .length
          );
        },
        skillAnimation: true,
      ...
      PSchanhuo: {
        ...
        filter: function (event, player) {
          return (
            !player.storage.PSchanhuo &&
            !player.getHistory("useSkill", (evt) => evt.skill === "PShuaiji")
              .length
          );
        },
        skillAnimation: true,
  ...
  PSqun_zhaoyun: {
    ...
    skills: {
      PSlongwei: {
        ...
        filter: function (event, player, name) {
          const target =
            name === "useCardToPlayer" ? event.target : event.player;
          return (
            event.card.name == "sha" &&
            target &&
        ...
        content: function () {
          "step 0";
          const target =
            event.triggername === "useCardToPlayer"
              ? trigger.target
              : trigger.player;
          ...
          if (
            ...
            result.cards &&
            result.cards.length &&
            get.color(trigger.card) === get.color(result.cards[0]) &&
            ["o", "d"].includes(get.position(result.cards[0]))
          ) {
        ...
        mod: {
          ...
          selectTarget: function (card, player, range) {
            if (
              card.name == "sha" &&
              card.storage &&
              card.storage.PSqiangshu &&
              range[1] != -1
            ) {
              try {
                range[1] = get.cardNameLength(
                  card.cards.length ? card.cards[0] : card
                );
              } catch (e) {
                range[1] =
                  lib.translate[
                    (card.cards.length ? card.cards[0] : card).name
                  ].length;
              }
            }
          },
        ...
        precontent: function () {
          ...
          player
            ...
            .filter((evt) => {
              ...
              return (
                evt.getParent().skill === "PSluoyi" &&
                player.countCards("e") <= evt.player.countCards("e")
              );
            })
            .then(() => {
  ...
  PSgongsunzan: {
    ...
    skills: {
      PSdangkou: {
        ...
        filter: function (event, player) {
          return (
            player.countCards("hes") > 0 && !player.hasSkill("PSdangkou_used")
          );
        },
        async content(event, trigger, player) {
          const { control } = await player
            ...
            if (
              game.hasPlayer(function (current) {
                return (
                  ...
                  !current.hasShan() &&
                  current.hp +
                  current.countCards("h", { name: ["tao", "jiu"] }) >
      ...
      PSxujin: {
        ...
        getSuitNumber: function (show) {
          ...
          game.getGlobalHistory("useCard", (evt) => {
            if (lib.suit.includes(get.suit(evt.card)))
        ...
        mod: {
          targetInRange: function (card, player, target) {
            if (
              card.name == "sha" &&
              lib.skill.PSxujin.getUseNumber(player) >= 1
            )
              return true;
          },
        },
        ...
        intro: {
          ...
          markcount: function (storage, player) {
            if (player.storage.PSxujin)
              return (
                game.getGlobalHistory("useCard").length - player.storage.PSxujin
              );
        ...
        filter: function (event, player, name) {
          ...
          return (
            ...
            lib.skill.PSxujin.getUseNumber(player) >= 2
          );
        },
        content: function () {
          trigger.getParent().directHit.addArray(game.players);
          ...
          player
            ...
            .filter((evt) => {
              return (
                evt.getParent(2) === trigger.getParent() &&
                lib.skill.PSxujin.getUseNumber(player) >= 3 &&
            ...
            .then(() => {
              ...
              game.log(
                trigger.card,
                "造成伤害+",
                lib.skill.PSxujin.getSuitNumber()
              );
            });
          player
            .when("useCardAfter")
            .filter((evt) => {
              return (
                get.name(evt.card) === "sha" &&
                lib.skill.PSxujin.getUseNumber(player) >= 4
              );
            })
            .then(() => {
              player.drawTo(player.maxHp);
  ...
  PSshen_simayi: {
    ...
    skills: {
      PSrenjie: {
        mod: {
          aiOrder: (player, card, num) => {
            if (num <= 0 || typeof card !== "object" || !player.isPhaseUsing())
              return num;
            if (
              lib.skill.PSrenjie.derivation.filter(
                (skill) => !player.hasSkill(skill)
              )
                .length /* && player.getUseValue(card) < Math.min(4, player.hp * player.hp / 4) */
            )
              return 0;
          },
        },
        ...
        init: function (player, skill) {
          changeSkillAudio("reguicai", player.name, "jilue_guicai");
          changeSkillAudio("PSfangzhu", player.name, "jilue_fangzhu");
          changeSkillAudio("PSjizhi", player.name, "jilue_jizhi");
          changeSkillAudio("PSzhiheng", player.name, "jilue_zhiheng");
          changeSkillAudio("PSwansha", player.name, "wansha_shen_simayi");
        },
        derivation: [
          "reguicai",
          "PSfangzhu",
        ...
        async content(event, trigger, player) {
          ...
          const num =
            event.triggername === "damageEnd"
              ? trigger.num
              : trigger.getl(player).cards2.length;
          const skills = lib.skill.PSrenjie.derivation.filter(
            (skill) => !player.hasSkill(skill)
          );
          ...
          const choiceList = skills.map((skill, i) => [
            i,
            get.translation(skill) + "：" + get.translation(skill + "_info"),
          ]);
          const { links } = await player
            .chooseButton([
              `忍戒：请选择获得${get.cnNumber(num)}个技能`,
      ...
      PSbaiyin: {
        ...
        filter(event, player) {
          return (
            lib.skill.PSrenjie.derivation.every((skill) =>
              player.hasSkill(skill)
            ) && event.getParent(2).skill === "PSrenjie"
          );
        },
        content: async function (event, trigger, player) {
    ...
    skills: {
      PSshuangjia: {
        ...
        filter: function (event, player) {
          return (
            (event.name !== "phase" || game.phaseNumber === 0) &&
            event.getParent(2).skill != "dcbeifen"
          );
        },
        content: function () {
    ...
    skills: {
      PSfangzhu: {
        mod: {
          targetEnabled: function (card, player, target, now) {
            if (
              target.isTurnedOver() !== player.isTurnedOver() &&
              _status.currentPhase === player
            )
              return false;
          },
        ...
        content: function () {
          ...
          if (result.bool) {
            ...
            player
              ...
              .set(
                "choice",
                get.attitude(player, event.target) > 0 || event.target.hp > 1
                  ? list[0]
                  : list[1]
              )
              .set(
                "prompt",
                `放逐：令${get.translation(event.target)}执行一项`
              );
          } else event.finish();
          ("step 2");
          if (result.control) {
    ...
    skills: {
      PSlingren: {
        ...
        getSkill: function (player) {
          ...
          list = list.filter(
            (char) =>
              lib.character[char][4].includes("zhu") &&
              lib.character[char][1] === "wei"
          );
          for (var i of list) {
            if (i.indexOf("gz_jun") == 0) continue;
            for (var j of lib.character[i][3]) {
        ...
        subSkill: {
          damage: {
            ...
            filter: function (event, player) {
              return (
                player.storage.PSlingren_damage &&
                player.storage.PSlingren_damage.includes(event.player)
              );
            },
            async content(event, trigger, player) {
              ...
              const { control } = await player
                ...
                .set(
                  "prompt",
                  `凌人：是否改变对${get.translation(trigger.player)}造成的伤害？`
                )
                .set("ai", function () {
        ...
        async content(event, trigger, player) {
          const filter = function (target) {
            return (
              target.countCards("h") !== 0 &&
              player.storage.PSlingren_multi &&
              !player.storage.PSlingren_multi.includes(target) &&
              target !== player
            );
          };
          let Targets = (player.storage.PSlingren_multi || [])
            .slice(0)
            .remove(player);
          ...
          Targets = Targets.filter(
            (current) => current.isAlive() && current.isIn()
          ).sortBySeat();
  ...
  PSdian_huanggai: {
    ...
    skills: {
      PSshiming: {
        ...
        forced: true,
        init: function (player) {
          changeSkillAudio("xinfu_qinguo", player.name, "zhaxiang");
          changeSkillAudio(
            "xinfu_qinguo_recover",
            player.name,
            "zhaxiang"
          );
          changeSkillAudio("new_repaoxiao", player.name, "sbzhaxiang");
          changeSkillAudio("buqu", player.name, "rekurou");
        },
  ...
  PSliaohua: {
    ...
    skills: {
      PSdangxian: {
        ...
        forced: true,
        content: function () {
          trigger.phaseList.splice(
            trigger.num,
            0,
            "phaseDraw|xindangxian",
            "phaseUse|xindangxian"
          );
        },
    ...
    skills: {
      PStieji: {
        ...
        content: function () {
          ...
          if (
            player.hasHistory("custom", (evt) => evt.PStieji_target === target)
          ) {
            event.finish();
            return;
          }
          ...
          if (result.bool) {
            ...
            const str =
              event.choice === "db_def1"
                ? "摸两张牌"
                : `获得${get.translation(target)}的一张牌`;
            player
              .chooseBool(
                `是否${str}？然后你本回合对${get.translation(target)}发动〖铁骑〗时不能与其进行谋弈`
              )
              .set("ai", (event, player) => {
              ...
              .set(
                "canGain",
                result.player === "db_def1" ||
                (result.player === "db_def2" &&
                  target.countDiscardableCards(player, "he") > 0)
              );
          }
          ("step 2");
  ...
  PSqun_machao: {
    ...
    skills: {
      PSzhuiming: {
        ...
        filter: function (event, player) {
          return (
            event.card.name == "sha" && get.distance(player, event.target) === 1
          );
        },
        check: function (event, player) {
          return get.attitude(player, event.target) <= 0;
          ...
          if (result.control) {
            switch (result.index) {
              ...
              case 1:
                if (
                  trigger.cards.length &&
                  ["o", "d"].includes(get.position(trigger.cards[0]))
                )
          ...
          skillTagFilter: function (player, tag, arg) {
            if (tag == "unequip") {
              if (
                ...
                arg.card.name === "sha" &&
                arg.target &&
                player.storage.PSzhuiming &&
                player.storage.PSzhuiming.includes(arg.target)
              )
                return false;
            }
          },
      ...
      PSjinge: {
        mod: {
          ...
          cardUsable: function (card, player, num) {
            if (
              ...
              card.cards.length === 1 &&
              get.name(card) === "sha" &&
              get.cardNameLength({ name: card.cards[0].name }) === 2
            )
              return Infinity;
          },
        },
        ...
        filter: function (event, player) {
          ...
          return (
            get.name(event.card) === "sha" &&
            get.cardNameLength(event.card.cards[0]) === 2
          );
        },
        content: function () {
          trigger.addCount = false;
  ...
  PSluji: {
    ...
    skills: {
      ...
      PSzhenglun: {
        ...
        check: function (event, player) {
          return (
            player.countCards("h") >= 2 || player.skipList.includes("phaseUse")
          );
        },
        content: function () {
          trigger.cancel();
  ...
  PSlvbu: {
    ...
    skills: {
      ...
      PSbaifu: {
        ...
        check: function (event, player) {
          const target = game.findPlayer(
            (current) =>
              current.storage.PSbaifu_yifu &&
              current.storage.PSbaifu_yifu.includes(player)
          );
        ...
        filter: function (event, player) {
          if (event.player === player) return false;
          return (
            !event.player.storage.PSbaifu_yifu ||
            !event.player.storage.PSbaifu_yifu.includes(player)
          );
        },
        async content(event, trigger, player) {
          const yifu = game.filterPlayer(
            (current) =>
              current.storage.PSbaifu_yifu &&
              current.storage.PSbaifu_yifu.includes(player)
          );
        ...
        subSkill: {
          yifu: {
            intro: {
              content: function (storage, player) {
                return player.storage["PSbaifu_yifu"].reduce(
                  (a, b) => a + `我儿${get.translation(b)}天下无双！<br>`,
                  ""
                );
              },
              markcount: function (storage, player) {
      ...
      PSpanshi: {
        ...
        logTarget: (event, player) =>
          event.name === "damage" ? event.player : player,
        content: function () {
          if (event.triggername === "damageBegin1") {
            ...
            player
              ...
              .filter(
                (evt, player) =>
                  evt.player.storage.PSbaifu_yifu &&
                  evt.player.storage.PSbaifu_yifu.includes(player)
              )
              .then(() => {
                game.playAudio(
                  "..",
                  "extension",
                  "PS武将/audio/die",
                  ["PSpanshi_kill1", "PSpanshi_kill2"].randomGet()
                );
              });
            game.playAudio(
              "..",
              "extension",
              "PS武将/audio/skill",
              ["PSpanshi1", "PSpanshi2"].randomGet()
            );
          } else {
            trigger.player.getHistory("custom").push({ PSpanshi_use: true });
            trigger.targets.remove(player);
            if (
              trigger.targets.every((target) =>
                player.canUse(trigger.card, target, true, true)
              )
            ) {
              trigger.player = player;
              game.playAudio(
                ...
                "extension",
                "PS武将/audio/skill",
                ["PSpanshi_use1", "PSpanshi_use2"].randomGet()
              );
              trigger.noai = true;
              game.delay(0.5);
            }
  ...
  PSshen_zuoci: {
      ...
      PSshen_huashen: {
        ...
        filter: function (event, player, name) {
          ...
          return (
            ...
            player.storage.PSshen_huashen.character.length > 0
          );
        },
        direct: true,
        content: function () {
          ...
          if (player.isOnline2()) {
            player.send(
              function (cards, id) {
                var dialog = ui.create.dialog(
                  '###是否发动【化身】？###<div class="text center">选择一张“魂”作为你的表象武将</div>',
                  [
                    cards,
                    (item, type, position, noclick, node) =>
                      lib.skill.PSshen_huashen.$createButton(
                        item,
                        type,
                        position,
                        noclick,
          ...
          next.set("ai", function (button) {
            ...
            const num = rarity.legend.includes(button.link)
              ? 4
              : rarity.epic.includes(button.link)
                ? 3
                : rarity.rare.includes(button.link)
                  ? 2
                  : 1;
            return num;
          });
          ...
          if (result.bool) {
            ...
            const name =
              player.name2 === "PSshen_zuoci" ? player.name2 : player.name;
            ...
            //获得技能
            const skills = get
              .character(event.card, 3)
              .filter(function (skill) {
                const categories = get.skillCategoriesOf(skill);
                return !categories.some((type) =>
                  lib.skill.PSshen_huashen.bannedType.includes(type)
                );
              });
            if (skills.length) {
        ...
        subSkill: {
          use: {
            ...
            filter: function (event, player) {
              if (
                !player.storage.PSshen_huashen.invalid ||
                !player.storage.PSshen_huashen.character.length
              )
                return false;
              for (var i of lib.inpile) {
                var type = get.type2(i);
                if (
                  ...
                  !player.hasHistory(
                    "custom",
                    (evt) =>
                      evt.PSshen_huashen_use && evt.card && evt.card.name === i
                  ) &&
                  event.filterCard(
              ...
              backup: function (links, player) {
                return {
                  ...
                  onuse: function (links, player) {
                    ...
                    next.links = links;
                    next.setContent(
                      lib.skill.PSshen_huashen_use.chooseHuashenCharacter
                    );
                  },
                };
              },
            ...
            chooseHuashenCharacter: function () {
              ...
              if (player.isOnline2()) {
                player.send(
                  function (cards, id) {
                    var dialog = ui.create.dialog(
                      '###是否发动【化身】？###<div class="text center">选择展示一张武将牌上的“魂”并将其置入剩余武将牌堆</div>',
                      [
                        cards,
                        (item, type, position, noclick, node) =>
                          lib.skill.PSshen_huashen.$createButton(
              ...
              if (result.bool) {
                player
                  .getHistory("custom")
                  .push({ card: event.links.card, PSshen_huashen_use: true });
                ...
                lib.skill.PSshen_huashen.removeHuashen(
                  player,
        ...
        addHuashen: function (player) {
          ...
          for (let i = 0; i < _status.characterlist.length; i++) {
            ...
            if (
              ...
              name.indexOf("key_") == 0 ||
              name.indexOf("sp_key_") == 0 /* || get.is.double(name) */ ||
              lib.skill.PSshen_huashen.banned.includes(name) ||
              player.storage.PSshen_huashen.character.includes(name)
            )
              continue;
            let skills = lib.character[name][3].filter((skill) => {
              const categories = get.skillCategoriesOf(skill);
        ...
        filter: function (event, player, name) {
          ...
          if (name === "dyingAfter")
            return (
              event.player !== player &&
              event.player.isAlive() &&
              !player.storage.PSshen_huashen.dyingAfter
            );
        ...
        async content(event, trigger, player) {
          ...
          switch (control) {
            case "选项一": {
              ...
              // player.setAvatar('PSshen_zuoci', 'PSshen_zuoci', true, true);
              if (
                player.additionalSkills.PSshen_huashen &&
                player.additionalSkills.PSshen_huashen.length
              ) {
              ...
              game.log(
                player,
                "移去了表象武将",
                "#y" + get.translation(player.storage.PSshen_huashen.current)
              );
          ...
          await player.recoverTo(
            !storage.disable + !storage.damage + !storage.dyingAfter + 1
          );
          trigger.cancel();
        },
      },
    ...
    translate: {
      ...
      "#ext:PS武将/audio/skill/PSshen_jihun1":
        "人死其魂，复归于气，汲其气修吾身。",
    ...
    dynamicTranslate: {
      PSshen_jihun: function (player) {
        ...
        if (player.storage.PSshen_huashen?.dyingAfter)
          str2 = `<del>${str2}</del>`;
      ...
      PSjixi: {
        ...
        filter: function (event, player) {
          ...
          for (var i = player.actionHistory.length - 1; i >= 0; i--) {
            ...
            if (
              _status.globalHistory[i].changeHp.some(
                (evt) =>
                  evt.player == player &&
                  evt.getParent().name == "loseHp" &&
                  evt.getParent(2).name == "PSshelian"
              )
            )
              return false;
            else {
          ...
          if (
            ...
            if (zhu && zhu != player && zhu.skills) {
              for (var i = 0; i < zhu.skills.length; i++) {
                if (
                  lib.skill[zhu.skills[i]] &&
                  lib.skill[zhu.skills[i]].zhuSkill
                ) {
      ...
      PSrebolan: {
          ...
          for (var i of list) {
            ...
            for (var j of lib.character[i][3]) {
              ...
              if (
                ...
                skill.dutySkill ||
                skill.chargeSkill ||
                lib.skill.PSrebolan.banned.includes(j)
              )
                continue;
              if (
                skill.init ||
        ...
        async content(event, trigger, player) {
          ...
          const { bool, targets } = await player
            .chooseTarget(
              `令一名角色获得【${get.translation(control)}】直到其回合结束`,
              function (card, player, target) {
                return true;
              }
            )
        ...
        subSkill: {
          g: {
            ...
            usable: 1,
            prompt:
              "出牌阶段限一次。你可以令一名有〖博览〗的角色从六个描述中包含“出牌阶段”的技能中选择一个，你获得此技能直到此阶段结束。",
            filter: function (event, player) {
              return game.hasPlayer(function (current) {
            ...
            selectTarget: function () {
              if (
                game.countPlayer((current) => {
                  return lib.skill.PSrebolan_g.filterTarget(
                    null,
                    _status.event.player,
                    current
                  );
            ...
            content: function () {
              ...
              if (target.isIn() && player.isIn()) {
                ...
                target
                  ...
                  .set(
                    "prompt",
                    "博览：请选择令" + get.translation(player) + "获得的技能"
                  )
                  .set("ai", () => {
                    var list = _status.event.controls.slice();
                    return list.sort((a, b) => {
            ...
            ai: {
              order: function (item, player) {
                ...
                var list = game.filterPlayer((current) =>
                  lib.skill.PSrebolan_g.filterTarget(null, player, current)
                );
              ...
              result: {
                player: function (player, target) {
                  if (player.hasUnknown())
                    return player.hp + player.countCards("h") / 4 - 5 > 0
                      ? 1
                      : 0;
      ...
      PSguangu: {
        ...
        async content(event, trigger, player) {
          ...
          const { cards } = await player
            ...
            .set(
              "prompt",
              `观骨：观看${get.translation(target)}的至多${get.cnNumber(num)}张牌`
            )
            .set("ai", (button) => {
              ...
              function handleGaintag(card) {
                setTimeout(() => {
                  try {
                    ...
                    const gaintag =
                      get.owner(source) === target &&
                      _status.event.moved[0].includes(source);
                    card.node.gaintag.innerHTML = gaintag
                      ? "被交换"
                      : get.owner(source) === target
                        ? "未被交换"
                        : "";
          ...
          const filterCards = cards.filter(
            (card) =>
              !moved[1].includes(card) && player.hasUseTarget(card, null, false)
          );
  ...
  PSshen_nanhualaoxian: {
    ...
    skills: {
      PSnandou: {
        ...
        hiddenCard: function (player, name) {
          ...
          return (
            type == "trick" &&
            player.countCards("hse", (card) => get.type(card) === "basic") > 0
          );
        },
        ai: {
          fireAttack: true,
      ...
      PSbeidou: {
        ...
        onremove: true,
        filter: function (event, player) {
          if (!player.countCards("hse", (card) => get.type(card) === "equip"))
            return false;
          for (var i of lib.inpile) {
            var type = get.type(i);
            if (
        ...
        hiddenCard: function (player, name) {
          ...
          return (
            type == "basic" &&
            player.countCards("hse", (card) => get.type(card) === "equip") > 0
          );
        },
        ai: {
          fireAttack: true,
      ...
      PSwuji: {
        ...
        enable: ["chooseToUse", "chooseToRespond"],
        filter: function (event, player) {
          if (!player.countCards("hse", (card) => get.type2(card) === "trick"))
            return false;
          for (var i of lib.inpile) {
            var type = get.type(i);
            if (
        ...
        hiddenCard: function (player, name) {
          ...
          return (
            type == "basic" &&
            player.countCards("hse", (card) => get.type2(card) === "trick") > 0
          );
        },
        ai: {
          fireAttack: true,
          ...
          skillTagFilter: function (player) {
            if (
              !player.countCards("hse", (card) => get.type2(card) === "trick")
            )
              return false;
          },
          order: 1,
      ...
      PSdaogu: {
        ...
        filter(event, player) {
          ...
          const skills = player
            ...
            .filter(
              (skill) =>
                !skill.startsWith("player_when_") && skill !== "PSdaogu"
            );
          if (!skills.includes(event.skill)) return false;
          return (
            player.getAllHistory("useSkill", (evt) =>
              skills.includes(evt.skill)
            ).length %
            num ===
            0
          );
        ...
        getResetableSkills(current) {
          ...
          for (const skill of skills) {
            ...
            if (typeof info.usable == "number") {
              if (
                current.hasSkill("counttrigger") &&
                current.storage.counttrigger[skill] &&
                current.storage.counttrigger[skill] >= 1
              ) {
                resetSkills.add(skill);
              }
              if (
        ...
        async content(event, trigger, player) {
          ...
          if (
            game.players.every((cur) => getResetableSkills(cur).length === 0)
          ) {
            choices.remove("选项二");
            choiceList[1] =
              '<span style="opacity:0.5; ">' + choiceList[1] + "</span>";
          ...
          if (
            game.players
              ...
              .every((skill) => player.hasSkill(skill))
          ) {
            choices.remove("选项三");
            choiceList[2] =
              '<span style="opacity:0.5; ">' + choiceList[2] + "</span>";
            ...
            ({ control } = await player
              ...
              .set("ai", () => {
                if (
                  player
                    .getSkills(true, false, false)
                    .filter((skill) => !skill.startsWith("player_when_")) >= 3
                )
                  return "选项一";
                else if (
                  (!player.storage.PSdaogu_nandou ||
                    !player.storage.PSdaogu_beidou) &&
                  choices.includes("选项二")
                )
                  return "选项二";
                return choices.at(-1);
              })
          ...
          switch (control) {
            ...
            case "选项二": {
              const { targets } = await player
                .chooseTarget("请选择要重置或恢复技能的角色", true)
              ...
              const { control } = await player
                .chooseControl(
                  target
                    ...
                    .filter((skill) => !skill.startsWith("player_when_"))
                    .filter((skill) =>
                      getResetableSkills(player).includes(skill)
                    )
                )
                .set("ai", () => {
            ...
            case "选项三": {
              const skill = game.players
                ...
                .randomGet();
              player
                .addTempSkills(skill, { player: "phaseEnd" })
                .set("$handle", (player, add, remove, event) => {
                  player.popup(add[0]);
                });
            }
          }
    ...
    translate: {
      ...
      "#ext:PS武将/audio/skill/PSwuji1":
        "此经所书晦涩难明，吾偶有所悟，愿为君陈之。",
      "#ext:PS武将/audio/skill/PSwuji2":
        "大音希声，大象无形，天理难明，以经合之。",
      "#ext:PS武将/audio/skill/PSdaogu1":
        "含气求道，祸福难料，且与阁下共参之。",
      "#ext:PS武将/audio/skill/PSdaogu2":
        "蕴气修德，其理易现，容吾为君讲解一二。",
      "#ext:PS武将/audio/die/PSshen_nanhualaoxian:die":
        "吾以天地为棺椁，以日月为连壁。",
    },
    dynamicTranslate: {
  ...
  PShr_caocao: {
        ...
        intro: {
          mark: function (dialog, storage, player) {
            if (storage) {
              ...
              if (skill) {
                dialog.addText(
                  `当你受到伤害后，或当你于回合外失去牌后，你获得技能〖${get.translation(skill)}〗。若已获得该技能，则：若因伤害触发，则对所有其他角色各造成一点伤害，否则弃置所有其他角色各一张牌。`,
                  false
                );
                dialog.add(
                  '<div><div class="skill">【' +
        ...
        async content(event, trigger, player) {
          ...
          if (player.storage.PSfaxiao) {
            ...
            game.broadcastAll(
              ...
              "../extension/PS武将/audio/skill/PSfaxiao"
            );
            const skill = player.storage.PSfaxiao_pending;
            if (skill) {
    ...
    dynamicTranslate: {
      PSfaxiao: function (player) {
        if (!player.storage.PSfaxiao)
          return '转换技，当你受到伤害后，或当你于回合外失去牌后，<span class="bluetext">阳：你可以念一句文本包含“哼”、“呵”、“哈”的台词（随机三个自选其一）；</span>阴：你获得你上一次所念的台词所属的技能。若已获得该技能，则：若因伤害触发，则对所有其他角色各造成一点伤害，否则弃置所有其他角色各一张牌。';
  ...
  PSshenpei: {
    ...
    skills: {
      ...
      PSliezhi: {
        ...
        filter: function (event, player) {
          return (
            player.countDiscardableCards(player, "h") > 0 &&
            player.hasUseTarget("wanjian")
          );
        },
        check(event, player) {
          ...
          const basicNum = player.getDiscardableCards(
            player,
            "h",
            (card) => get.type(card) === "basic"
          ).length;
          const trickNum = player.getDiscardableCards(
            player,
            "h",
            (card) => get.type2(card) === "trick"
          ).length;
          const equipNum = player.getDiscardableCards(
            player,
            "h",
        ...
        async content(event, trigger, player) {
          ...
          const types = [];
          const basic = player.getDiscardableCards(
            player,
            "h",
            (card) => get.type(card) === "basic"
          );
          const trick = player.getDiscardableCards(
            player,
            "h",
            (card) => get.type2(card) === "trick"
          );
          const equip = player.getDiscardableCards(
            player,
            "h",
            (card) => get.type(card) === "equip"
          );
      ...
      PSxieshou: {
        ...
        async cost(event, trigger, player) {
          ...
          const { bool, links } = await player
            .chooseCardButton(
              `协守：是否将其中至多${get.cnNumber(num)}张牌交给同势力或拥有〖协守〗的角色`,
              false,
              cards,
              [1, num]
            )
        ...
        async content(event, trigger, player) {
          ...
          const { targets } = await player
            ...
            .set("filterTarget", function (card, player, target) {
              return (
                target.hasSkill("PSxieshou") || target.group === player.group
              );
            })
            .set("ai", function (target) {
        ...
        subSkill: {
            ...
            filter(event, player, name) {
              if (
                !game.hasPlayer(
                  (cur) => cur.hasSkill("PSxieshou") && cur !== player
                )
              )
                return false;
              return (
                name === "respondEnd" ||
                (event.card && ["shan", "wuxie"].includes(get.name(event.card)))
              );
            },
            async cost(event, trigger, player) {
              event.result = await player
                .chooseTarget(
                  "协守：是否令一名拥有〖协守〗的其他角色摸一张牌？"
                )
                .set("filterTarget", function (card, player, target) {
  ...
  PSsb_zhangfei: {
      ...
      PSsheming: {
        ...
        async content(event, trigger, player) {
          ...
          if (player.storage.PSsheming) {
            ...
            const use_card = new lib.element.VCard({
              name: "sha",
              isCard: true,
            });
            const use_targets = [];
        ...
        ai: {
          ...
          result: {
            target(player, target) {
              ...
              var eff =
                get.effect(target, { name: "shunshou" }, player, player) * num;
              if (eff * num > 0) {
                return eff / 10;
              }
              return eff;
        ...
        subSkill: {
          ...
          clear: {
            ...
            onremove(player) {
              game.countPlayer2(function (current) {
                current.removeSkill(
                  "PSkuangli_" + player.playerid + "_fengyin"
                );
              });
            },
          },
    ...
    replace: ["PSsb_zhangfei", "PSzhangfei"],
    sort: sort.PScharacter_shu,
    translate: {
      PSsb_zhangfei: "PS谋张飞",
      ...
      PSsheming: "慑酩",
      PSsheming_info:
        "转换技，出牌阶段限两次，阳：你可以获得至多两名其他角色各一张手牌，然后视为使用一张【酒】，你以此法获得的牌均视为不计入手牌上限的【酒】；阴：你可以交给至多两名其他角色各一张手牌，视为使用一张以这些角色为目标的【杀】，这些角色以此法获得的牌均视为不计入手牌上限的【杀】。",
      PSkuangli: "狂醴",
      PSkuangli_info:
        "锁定技，当你使用【酒】后，若你本回合使用的下一张牌为【杀】，你令此【杀】无距离和次数限制，且目标角色非锁定技失效直到此【杀】结算结束。",
      PSjinxi: "烬息",
      PSjinxi_info:
        "锁定技，当你使用【杀】后，若你本回合使用的上一张牌为【酒】，你摸一张牌并失去一点体力，然后你令此【酒】不计入本回合使用次数。",
    },
    dynamicTranslate: {
      PSsheming: function (player) {
        if (!player.storage.PSsheming)
          return '转换技，出牌阶段限两次，<span class="bluetext">阳：你可以获得至多两名其他角色各一张手牌，然后视为使用一张【酒】，你以此法获得的牌均视为不计入手牌上限的【酒】；</span>阴：你可以交给至多两名其他角色各一张手牌，视为使用一张以这些角色为目标的【杀】，这些角色以此法获得的牌均视为不计入手牌上限的【杀】。';
  ...
  PShanbingjiangirl: {
    ...
    skills: {
      PShanying: {
        ...
        prompt2(event, player) {
          ...
          const num = Math.min(
            event.player.countDiscardableCards(player, "he"),
            count
          );
      ...
      PSaoshuang: {
        ...
        filter(event, player) {
          ...
          return (
            event.type == "discard" &&
            event.getl(event.player).cards2.length > 0
          );
        },
        async content(event, trigger, player) {
          ...
          const count = lib.skill.PShanying.getShaCount();
          const list = [
            `令${get.translation(trigger.player)}受到一点无来源伤害`,
            `摸${get.cnNumber(count)}张牌`,
          ];
          const { index } = await player
            .chooseControlList(list, false)
      ...
      PSdouxie: {
        ...
        mod: {
          cardUsable: function (card) {
            if (card.storage && card.storage.PSdouxie) return Infinity;
          },
        },
        selectCard: -1,
        filterCard: () => false,
    ...
    translate: {
      ...
      PShanying: "寒英",
      PShanying_info:
        "你即将造成伤害时，你可以防止此伤害并弃置该角色至多X张牌（X弃牌堆中“杀”的数量）。",
      PSaoshuang: "傲霜",
      PSaoshuang_info:
        "一名角色于你的回合被弃置牌后，你可以选择一项；1，该角色受到一点无来源的伤害：2，你摸X张牌（X弃牌堆中“杀”的数量）。",
      PSdouxie: "斗雪",
      PSdouxie_info:
        "出牌阶段限X次，你可以视为对一名其他角色使用一张“冰”杀（不计入使用次数且无次数限制，X弃牌堆中“杀”的数量）。",
    },
    rank: rank.legend,
  },
  PSsb_zuoci: {
    ...
    skills: {
      PShuanqu: {
        ...
        init: function (player, skill) {
          ...
          game.updateRenku = function (...args) {
            const result = lib.skill.PShuanqu.original_game_updateRenku.apply(
              this,
              args
            );
        ...
        checkRenku(player) {
          const idList = player
            .getCards("s", (card) => card.hasGaintag("PShuanqu"))
            .map((i) => i._cardid);
          const cards = _status.renku
            .slice()
          ...
          const cards3 = player
            .getCards("s", (card) => card.hasGaintag("PShuanqu"))
            .filter((c) => !_status.renku.find((r) => r.cardid === c._cardid));
          if (cards3.length) {
            if (player.isOnline2()) {
              player.send(
        ...
        ai: {
          effect: {
            player_use(card, player, target) {
              if (get.itemtype(card) == "card") {
                if (card.hasGaintag("PShuanqu")) return [1, 1];
              } else if (card.isCard && card.cards) {
                if (card.cards.some((card) => card.hasGaintag("PShuanqu")))
                  return [1, 1];
              }
            },
          },
          ...
          use: {
            ...
            filter(event, player) {
              const cards = player.getCards(
                "s",
                (card) => card.hasGaintag("PShuanqu") && card._cardid
              );
              return (
                event.cards &&
            ...
            async content(event, trigger, player) {
              ...
              const cards = [];
              cards.addArray(
                _status.renku.slice().filter((i) => idList.includes(i.cardid))
              );

              const cards2 = [];
              for (const card of trigger.cards) {
                const cardx = cards.find(
                  (cardx) => cardx.cardid === card._cardid
                );
                if (cardx) {
                  cards2.push(cardx);
                } else {
              ...
              const cards3 = trigger.cards
                .slice()
                .filter((card) => card.hasGaintag("PShuanqu"));
      ...
      PSxicao: {
        ...
        filter(event, player, name) {
          ...
          if (player.getStorage("PSxicao_used").includes(name.slice(9)))
            return false;
        ...
        async cost(event, trigger, player) {
          ...
          if (event.triggername == "useCardToPlayered") {
            const nogoon = function () {
              if (trigger.target === player) {
                if (get.type(trigger.card, player) === "equip") return false;
                if (player.hp === player.maxHp && get.name(trigger.card, player) === "tao") return false;
              }
              if (get.type(trigger.card, player) === "delay") return false;
              if (get.value(trigger.card, player) < 3) return false;
              return true;
            }
            const { bool, links } = await player
              .choosePlayerCard(
                ...
                "he",
                false,
                `戏曹：是否将${get.translation(target)}的一张牌置入仁库中，然后令${get.translation(trigger.card)}额外结算一次`
              )
              .set("nogoon", nogoon)
              .set("ai", card => {
            ...
            const { bool, cards } = await player
              .chooseCard(
                "he",
                false,
                `戏曹：是否将你的一张牌置于仁库中，然后令${get.translation(trigger.card)}对你无效`
              )
              .set("ai", card => {
                const player = get.player();
                if (trigger.player === player) {
                  if (player.countCards("h") > 5 && _status.renku.length < 5 && get.value(trigger.card) <= 5) return 3 - get.value(card);
                  return 0;
                }
                return 12 - get.value(card);
              })
    ...
    translate: {
      ...
      PSxicao: "戏曹",
      PSxicao_info:
        "每回合各限一次，当你使用牌指定/成为牌的唯一目标后，你可以将目标角色/你的一张牌置入仁区，令此牌额外结算一次/对你无效。",
    },
    rank: rank.epic,
  },
  PSpot_yuji: {
    ...
    skills: {
          ...
          global: {
            ...
            async content(event, trigger, player) {
              ...
              const { bool, cards } = await target
                .chooseToGive(
                  player,
                  `符济：是否交给${get.translation(player)}一张牌？`,
                  `其使用你给予的牌后，你也可以获得好处`,
                  1
                )
                .set("ai", (card) => {
                  ...
                  if (lib.card[card.name].ai?.tag?.damage) add += 3;
                  return (
      ...
      PSdaozhuan: {
        ...
        chooseButton: {
          dialog(event, player) {
            return ui.create.dialog("道转", [
              get.inpileVCardList((info) => get.type(info[2]) === "basic"),
              "vcard",
            ]);
          },
                ...
                player
                  ...
                  .then(() => {
                    ...
                    for (const card of trigger.cards) {
                      ...
                      if (
                        name === "sha" ||
                        get.translation(name).includes("杀")
                      ) {
                        hasSha = true;
                      } else if (
                        name === "shan" ||
                        get.translation(name).includes("闪")
                      ) {
                        hasShan = true;
                      }
        ...
        hiddenCard(player, name) {
          return (
            ...
            !player.hasSkill("PSdaozhuan_used")
          );
        },
        ai: {
          fireAttack: true,
            ...
            if (player && _status.event.type === "phase") {
              let max = 0,
                names = get.inpileVCardList((info) => {
                  ...
                  return (
                    get.type(name) !== "basic" &&
                    !player.hasSkill("PSdaozuan_used")
                  );
                });
              names = names.map(
                (namex) =>
                  new lib.element.VCard({ name: namex[2], nature: namex[3] })
              );
              names.forEach((card) => {
    ...
    translate: {
      ...
      PSfuji: "符济",
      PSfuji_info:
        "其他角色的出牌阶段限一次，其可以令你交给其一张牌（可以拒绝），称为“符济”。其使用“符济”牌时，此牌的牌面数值+1；然后此牌结算完毕后，你与其各摸一张牌，若此牌造成了伤害，你改为摸两张牌。",
      PSdaozhuan: "道转",
      PSdaozhuan_info:
        "你可以将一张牌当作任意基本牌使用。当你以此法使用牌后，若此牌对应的实体牌牌面信息包含：“杀”，你弃置当前回合角色的一张牌；“闪”，你摸一张牌；均不包含，此技能本回合失效。",
    },
    rank: rank.epic,
  },
  PSxiaoqiao: {
    info: {
      ...
      group: "wu",
      hp: 3,
      skills: ["PStianxiang", "xinhongyan", "PSpiaoling"],
      dieAudios: [],
      trashBin: [],
    },
    skills: {
      PStianxiang: {
        ...
        subSkill: {
            ...
            silent: true,
            filter(event, player) {
              if (!player.hasSkill("PSpiaoling")) return false;
              const parent = event.getParent(3);
              return parent.name === "PStianxiang" && parent.player === player;
            },
            async content(event, trigger, player) {
              player.storage.PSpiaoling ??= 2;
              player.storage.PSpiaoling++;
            },
          },
        },
      },
      PSpiaoling: {
        ...
        async content(event, trigger, player) {
          ...
          const num = player.storage.PSpiaoling ?? 2;
          trigger.player.storage.PSpiaoling_effect ??= num;
          trigger.player.addTempSkill("PSpiaoling_effect");
        },
        subSkill: {
          effect: {
            charlotte: true,
            ...
            async content(event, trigger, player) {
              const num = player.storage.PSpiaoling_effect ?? 2;
              trigger.num += num;
            },
            mod: {
              maxHandcard(player, num) {
                return num + (player.storage.PSpiaoling_effect ?? 2);
              },
            },
          },
        }
    ...
    translate: {
      PSxiaoqiao: "PS小乔",
      PStianxiang: "天香",
      PStianxiang_info: "当你受到伤害时，你可以弃置一张红桃牌，防止此伤害并选择一名其他角色，然后你选择一项：1.令其受到伤害来源对其造成的1点伤害，然后摸X张牌（X为其已损失体力值且至多为5）；2.令其失去1点体力，然后获得你弃置的牌。若其因此死亡且你拥有技能〖飘零〗，你令〖飘零〗的数字+1。",
      PSpiaoling: "飘零",
      PSpiaoling_info: "每回合限一次，一名角色的摸牌阶段开始前，你可以受到一点伤害，令其本回合的摸牌数和手牌上限+2。",
    },
    dynamicTranslate: {
      PSpiaoling: function (player) {
        const storage = player.storage.PSpiaoling ?? 2;
        return `每回合限一次，一名角色的摸牌阶段开始前，你可以受到一点伤害，令其本回合的摸牌数和手牌上限+${storage}。`;
      },
    },
    rank: rank.rare,
  },
  PSv_liubei: {
    ...
    skills: {
      PSlixian: {
        audio: "ext:PS武将/audio/skill:2",
        trigger: {
          player: ["changeHp", "phaseJieshuBegin"],
        },
        getList() {
          let list;
          if (get.mode() == "guozhan") {
        ...
        getWuhuSkills(list) {
          ...
          for (const i of wuhuList) {
            const skillsx = get.character(i).skills
              .filter(skill => {
                const info = get.info(skill);
        ...
        getShuSkills(list) {
          list = list.filter(i => get.character(i).group == "shu");
        ...
        getIndex(event, player, name) {
          return name === "changeHp" ? Math.abs(event.num) : 1;
        },
        getChoiceList(evtName) {
          const list = ["蜀势力", "五虎将"];
          if (evtName !== "PSlixian_use") list.push("cancel2");
          return list;
        },
        async cost(event, trigger, player) {
          const { control } = await player
            .chooseControl(lib.skill.PSlixian.getChoiceList(event.name))
            .set("ai", function () {
              return get.event().choice;
            })
            .set("prompt", `礼贤：${event.name === "PSlixian_use" ? "请你" : "你可以"}从“蜀势力”或“五虎将”中，选择至多5个技能依次分发`)
            .set("choice", ["蜀势力", "五虎将"].randomGet())
            .forResult();
          if (event.name === "PSlixian_use") return control;
          event.result = {
        ...
        async content(event, trigger, player) {
          const list = lib.skill.PSlixian.getList();
          const skills = (event.cost_data.control === "蜀势力"
            ? lib.skill.PSlixian.getShuSkills(list)
            : lib.skill.PSlixian.getWuhuSkills(list))
            .randomGets(5);

          const skillList = [];
          for (const skill of skills) {
            skillList.push([skill, '<div class="popup text" style="width:calc(100% - 10px);display:inline-block"><div class="skill">【' + get.translation(skill) + "】</div><div>" + lib.translate[skill + "_info"] + "</div></div>"]);
          }
          const next = player.chooseButton(["礼贤：请选择一至五项", [skillList, "textbutton"]]);
          next.set("forced", true);
          ...
          const result = await player.chooseTarget(`礼贤：将${links.map(s => "〖" + get.translation(s) + "〗").join("、")}依次分发给至多${get.cnNumber(links.length)}名角色`, [1, links.length], true)
            .set("targetprompt", target => {
              const count = ui.selected.targets.length;
              return get.translation(links[count - 1]);
            })
            .set("ai", target => {
              const att = get.attitude(_status.event.player, target);
          ...
          if (result.targets?.length) {
            ...
            const targetsx = targets.filter(target => target.group !== "shu");
            if (player.hasSkill("PSweiyi") && targetsx.length && !Object.keys(player.disabledSkills).includes("PSweiyi")) {
              for (const target of targetsx) {
                const { control } = await target
                  .chooseControl(["失去一个技能", "翻面"])
                  .set("ai", function () {
                    return get.event().choice();
                  })
                  .set("prompt", `礼贤：失去一个技能或者翻面`)
                  .set("choice", () => {
                    const target = get.event().player;
                    if (target.isTurnedOver()) return "翻面";
                    if (Math.random() <= 0.8) return "失去一个技能";
                    return "翻面";
                  })
                  .forResult();
                if (control === "翻面") {
                  ...
                  if (skills.length === 1) {
                    await target.removeSkills(skills[0]);
                    continue;
                  }
                  const next = target.chooseButton(["礼贤：选择失去1个技能", [skills, "skill"]]);
                  next.set("forced", true);
                  next.set("ai", button => {
                    const skill = button.link,
                      choice = get.event().choice;
                  ...
                  next.set(
                    "choice",
                    skills.sort((a, b) => {
                      return get.skillRank(b, "out") - get.skillRank(a, "out");
                    })[0]
                  );
                  const { links } = await next.forResult();
                  await target.removeSkills(links[0]);
                }
              }
            }
            for (let i = 0; i < targets.length; i++) {
              await targets[i].addSkills(links[i]);
            }
            await game.delayx();
          }
        },
        ...
        subSkill: {
          recover: {
            ...
            filter(event, player) {
              ...
              return game.hasPlayer(function (current) {
                return current !== player && current.hasSkill("PSweiyi") && current.isDamaged();
              });
            },
            usable: 1,
            prompt() {
              ...
              let str = "令" + get.translation(list);
              if (list.length > 1) {
                str += "中的一人";
              }
              str += "回复一点体力";
            ...
            filterTarget(card, player, target) {
              return target !== player && target.hasSkill("PSweiyi") && target.isDamaged();
            },
            selectTarget() {
              const player = get.player();
    ...
    translate: {
      PSv_liubei: "PS威刘备",
      "#ext:PS武将/audio/die/PSv_liubei:die": "醉泣江山君莫笑，似我刘备有几人。",
      PSv_liubei_prefix: "PS威",
      PSlixian: "礼贤",
      PSlixian_info: "你可以于以下时机发动〖礼贤〗：1. 出牌阶段限X次；2. 变化一点体力时；3. 结束阶段限一次（X为场上蜀势力角色的数量）。你从已开通的蜀势力的角色或从五虎将的技能库中选取至多五个并依次分发。",
      "#ext:PS武将/audio/skill/PSlixian1": "我将丹心酿烈酒，且取一觞慰风尘。",
      "#ext:PS武将/audio/skill/PSlixian2": "余酒尽倾江海中，与君共宴天下人。",
      PSweiyi: "威仪",
      PSweiyi_info: "威主技，每阶段限一次，其他蜀势力角色的出牌阶段可以令你回复一点体力。你对非蜀势力角色发动〖礼贤〗时，该角色须选择一项: 1. 失去一个技能；2.翻面。",
      "#ext:PS武将/audio/skill/PSweiyi1": "卿乃志同之股肱，国贼当前，可欲诛之。",
      "#ext:PS武将/audio/skill/PSweiyi2": "请君振炎汉之武运，除篡国之逆贼。",
    },
    rank: rank.legend,
  }
};
